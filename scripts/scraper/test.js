...
(async () => {
  ...
  const info = await p.evaluate(() => {
     const titleNode = Array.from(document.querySelectorAll('h1, h2, div')).find(el => el.innerText && el.innerText.includes('Volan detaylı'));
     const priceNode = Array.from(document.querySelectorAll('div, span')).find(el => el.innerText && el.innerText.includes('2.500'));
