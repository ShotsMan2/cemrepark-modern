  ...
  test("Complete journey from product to order placement", async ({ page }) => {
    ...
    if (await productLink.isVisible()) {
      ...
      const sizeButton = page
        .locator("button")
        .filter({ hasText: /^S$|^M$|^L$|^XL$|^36$|^38$|^40$/ })
        .first();
    ...
    await fillField(
      ['textarea[name="address"]', 'textarea[placeholder*="Adres"]', 'input[name="address"]'],
      "Atatürk Bulvarı No: 123 Daire: 4"
    );

    // Payment Info (Mock/Dummy) if present
    await fillField(
      ['input[name="cardNumber"]', 'input[placeholder*="Kart Numarası"]'],
      "4242424242424242"
    );
    await fillField(
      [
    ...
    await fillField(
      ['input[name="cardName"]', 'input[placeholder*="Kart Üzerindeki İsim"]'],
      "Playwright Tester"
    );
    ...
    // 6. Submit the order
    const submitOrderBtn = page
      .locator("button", { hasText: /Siparişi Onayla|Siparişi Tamamla|Ödeme Yap/i })
      .first();
    if (await submitOrderBtn.isVisible()) {
      ...
      await Promise.race([
        ...
        expect(
          page.locator("text=/Siparişiniz başarıyla alındı|Teşekkürler|Sipariş Özeti/i").first()
