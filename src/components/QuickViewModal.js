  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="glass-panel w-full max-w-4xl max-h-[90vh] overflow-y-auto clip-angled relative z-10 flex flex-col md:flex-row animate-fade-in">
        <button 
          onClick={onClose}
          className="absolute top-4 right-4 w-10 h-10 bg-black/50 rounded-full flex items-center justify-center text-white hover:text-neon-pink hover:bg-white transition-colors z-20"
        >
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
        </button>
        
        <div className="w-full md:w-1/2 h-[400px] md:h-[600px] relative clip-angled">
          <Image src={getValidImageUrl(product.gorsel || product.resim1)} alt={product.ad} fill className="object-cover" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" />
        </div>
        ...
        <div className="w-full md:w-1/2 p-8 flex flex-col justify-center">
          ...
          <div className="flex gap-4">
            <Link href={`/urundetay/${product.id}`} className="flex-1 text-center bg-transparent border border-neon-pink text-neon-pink px-6 py-4 font-bold uppercase tracking-widest hover:bg-neon-pink hover:text-white transition-colors clip-angled">
              {t("details")}
            </Link>
            <button 
