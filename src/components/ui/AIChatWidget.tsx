...
export function AIChatWidget() {
  ...
  const [messages, setMessages] = useState<{ role: "bot" | "user"; content: string; products?: any[]; quickReplies?: string[] }[]>([
    {
      role: "bot",
      ...
      {isOpen && (
        <div className="absolute bottom-16 right-0 w-[360px] h-[550px] max-h-[80vh] flex flex-col bg-white dark:bg-[#111] rounded-2xl shadow-2xl border border-gray-200 dark:border-white/10 overflow-hidden transform origin-bottom-right transition-all duration-300">
          ...
          <div className="bg-gradient-to-r from-neon-pink to-holo-gold p-4 flex justify-between items-center relative overflow-hidden">
            ...
            <button onClick={() => setIsOpen(false)} className="text-white/80 hover:text-white transition-colors p-2 hover:bg-white/10 rounded-full relative z-10">
                ...
                {msg.quickReplies && (
                  <div className="flex flex-wrap gap-2 mt-2 max-w-[90%]">
                    {msg.quickReplies.map(qr => (
                      <button key={qr} onClick={() => sendMessage(qr)} className="text-[11px] bg-neon-pink/10 hover:bg-neon-pink text-neon-pink hover:text-white dark:text-neon-pink px-3 py-1.5 rounded-full transition-colors border border-neon-pink/20">
                        {qr}
                      </button>
                    ))}
                  </div>
            ...
            {isLoading && (
              <div className="flex items-start">
                <div className="bg-white dark:bg-[#1a1a1a] border border-gray-100 dark:border-white/5 p-4 rounded-2xl rounded-tl-sm shadow-sm flex items-center gap-2">
                  ...
                  <div className="w-2 h-2 bg-holo-gold rounded-full animate-bounce" style={{ animationDelay: '0.2s' }}></div>
