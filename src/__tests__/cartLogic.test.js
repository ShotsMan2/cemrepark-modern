...
describe('cartLogic', () => {
  ...
  describe('validateCartStock', () => {
    ...
    it('should fail if requested quantity exceeds product stock (no variants)', () => {
      ...
      expect(result).toEqual({ isValid: false, message: 'Sadece 5 adet stok mevcuttur', availableStock: 5 });
    });
    ...
    it('should fail if requested quantity exceeds variant stock', () => {
      const product = {
        stok: 10,
        variations: [
          { renk: 'Kırmızı', beden: 'M', stok: 2 }
        ]
      };
      const cartItem = { id: 1, renk: 'Kırmızı', beden: 'M' };
      const result = validateCartStock(cartItem, 3, product);
      expect(result).toEqual({ isValid: false, message: 'Sadece 2 adet stok mevcuttur', availableStock: 2 });
    });
    ...
    it('should fail if selected variant is not found', () => {
      ...
      expect(result).toEqual({ isValid: false, message: 'Seçili varyant bulunamadı', availableStock: 0 });
    });
  ...
  describe('calculateCartDiscounts', () => {
    ...
    it('should not apply coupon if minCartValue condition is not met', () => {
      const cartItems = [{ fiyat: 200, quantity: 1 }];
      const coupon = { isActive: true, discountType: 'FIXED', discountValue: 50, minCartValue: 300 };
      const result = calculateCartDiscounts(cartItems, [], coupon);
      expect(result.subtotal).toBe(200);
      expect(result.discountAmount).toBe(0);
    ...
    it('should set free shipping flag correctly', () => {
      ...
      const promotions = [
        { isActive: true, type: 'FREE_SHIPPING', conditionType: 'MIN_CART_VALUE', conditionValue: 50, name: 'Ücretsiz Kargo' }
      ];
      const result = calculateCartDiscounts(cartItems, promotions);
    ...
    it('should not apply inactive or expired promotions', () => {
      ...
      const promotions = [
        ...
        { isActive: true, endDate: pastDate.toISOString(), type: 'FIXED_DISCOUNT', discountValue: 10, name: 'Süresi Dolmuş' },
        { isActive: true, startDate: futureDate.toISOString(), type: 'FIXED_DISCOUNT', discountValue: 10, name: 'Henüz Başlamamış' }
      ];
      const result = calculateCartDiscounts(cartItems, promotions);
      expect(result.discountAmount).toBe(0);
    });
