export const translations = {
  TR: {
    "collection": "Koleksiyon",
    "add_to_cart": "Sepete Ekle",
    "cart": "Sepet",
    "checkout": "Ödemeyi Tamamla",
    "total": "Toplam",
    "subtotal": "Ara Toplam",
    "shipping": "Kargo",
    "free": "Ücretsiz",
    "search_placeholder": "Ürün, kategori arayın...",
    "new_season": "YENİ SEZON KOLEKSİYONU",
    "free_shipping": "TÜM SİPARİŞLERDE ÜCRETSİZ KARGO",
    "discount": "İLK ALIŞVERİŞİNİZE ÖZEL %10 İNDİRİM",
    "installment": "KAPIDA NAKİT VE KREDİ KARTINA TAKSİT İMKANI",
    "size": "Beden",
    "color": "Renk",
    "price": "Fiyat",
    "details": "Tüm Detaylar",
    "quick_view": "Hızlı İncele",
    "no_products": "Eşleşen Ürün Bulunamadı",
    "clear_filters": "Filtreleri Temizle",
    "trending": "Trend Olanlar",
    "view_all": "Tümünü Gör",
    "buy_now": "Hemen Al",
    "contact": "İletişim",
    "about": "Hakkımızda",
    "hero_title": "Size çok yakışıcak! 💫",
    "hero_desc": "En trend tesettür takım, kap ve tunik modelleriyle tarzınızı yeniden keşfedin. Havale, EFT veya Kapıda Nakit/Kredi Kartı ile güvenle alışveriş yapın.",
    "explore_collection": "Koleksiyonu Keşfet",
    "search_results": "Arama Sonuçları",
    "total_products_listed": "Toplam {count} ürün listeleniyor.",
    "filters": "Filtreler",
    "price_range": "Fiyat Aralığı",
    "remove_filters": "Filtreleri Kaldır",
    "no_products_found": "Ürün Bulunamadı",
    "no_products_found_desc": "Aradığınız kriterlere uygun bir ürün stoklarımızda bulunmuyor.",
    "quick_view_desc": "Bu ürün Cemre Park özel koleksiyonunun bir parçasıdır. Göz alıcı tasarımı ve premium kumaş kalitesiyle öne çıkar. Detaylı incelemek için ürün sayfasına gidebilirsiniz.",
    "explore": "Keşfet",
    "corporate": "Kurumsal",
    "newsletter": "Bültene Katıl",
    "sets": "Takım",
    "tunics": "Tunik",
    "jackets": "Ceket",
    "dresses": "Elbise",
    "trousers": "Pantolon",
    "discover_collection": "Koleksiyonu Keşfet",
    "special_design": "Özel Tasarım",
    "premium_coats": "Premium Kaban Serisi",
    "explore_collection_btn": "Koleksiyonu İncele",
    "minimalist_dresses": "Minimalist Elbiseler",
    "modern_tunics": "Modern Tunikler",
    "testimonials_title": "Müşterilerimiz Ne Diyor?",
    "modern_tunics": "Modern Tunikler",
    "testimonials_title": "Müşterilerimiz Ne Diyor?",
    "testimonials_desc": "Yüzlerce mutlu müşterimizin arasına katılın.",
    "collections_menu": "KOLEKSİYONLAR",
    "discover_new_season": "YENİ SEZON KOLEKSİYONUMUZU KEŞFEDİN",
    "shopier_store": "Shopier Mağaza",
    "suits_you_well": "Size çok yakışacak! ✨",
    "add_to_favorites": "FAVORİLERE EKLE",
    "remove_from_favorites": "FAVORİLERDEN ÇIKAR",
    
    // YENİ EKLENEN ÇEVİRİLER
    "footer_desc": "Modern tesettür giyimde şıklık ve kalitenin adresi. En yeni kap, kaban, tunik ve takım modelleriyle tarzınızı yansıtın.",
    "explore_menu_new": "Yeni Gelenler",
    "explore_menu_best": "Çok Satanlar",
    "explore_menu_coats": "Kaban & Ceket",
    "explore_menu_accessories": "Aksesuar",
    "about_us": "Hakkımızda",
    "contact_us": "İletişim",
    "distance_selling": "Mesafeli Satış Sözleşmesi",
    "return_policy": "İade ve Değişim Koşulları",
    "privacy_policy": "Gizlilik Politikası",
    "newsletter_desc": "Özel koleksiyonlardan ilk sizin haberiniz olsun.",
    "email_placeholder": "E-posta Adresiniz...",
    "all_rights": "Tüm hakları saklıdır.",
    "secure_payment_infra": "Yüksek Güvenlikli Ödeme Altyapısı.",
    "secure_100": "100% Güvenli",
    "subscribe": "Abone Ol",
    "thanks": "Teşekkürler!",
    "newsletter_success": "Bültene başarıyla abone oldunuz.",
    "vip_family": "VIP AİLEMİZE KATILIN",
    "vip_desc": "Yeni sezon koleksiyonlarından, özel indirimlerden ve kampanyalardan ilk siz haberdar olun.",
    "my_cart": "Sepetim",
    "cart_empty": "Sepetiniz boş.",
    "continue_shopping": "Alışverişe Devam Et",
    "product_col": "Ürün",
    "quantity_col": "Adet",
    "order_summary": "Sipariş Özeti",
    "secure_shopping": "100% Güvenli Alışveriş",
    "grand_total": "Genel Toplam",
    "checkout_title": "Güvenli Ödeme",
    "shipping_info": "Teslimat Bilgileri",
    "full_name": "Ad Soyad",
    "phone": "Telefon",
    "city": "İl",
    "district": "İlçe",
    "address": "Açık Adres",
    "payment_info": "Ödeme Bilgileri",
    "card_number": "Kart Numarası",
    "dfsfs": "DFSFS",
    "Yeni Sezon": "Yeni Sezon",
    "Tükendi": "Tükendi",
    "Takım": "Takım",
    "Tunik": "Tunik",
    "Gömlek": "Gömlek",
    "Ceket": "Ceket",
    "Elbise": "Elbise",
    "Pantolon": "Pantolon",
    "fsf": "fsf",
    "selim": "Selim",
    "Volan Detaylı Etekli Takım": "Volan Detaylı Etekli Takım",
    "Büyük Beden Jakarlı Tunik": "Büyük Beden Jakarlı Tunik",
    "Modal Nakışlı Tunik": "Modal Nakışlı Tunik",
    "Modal Pantolonlu Takım": "Modal Pantolonlu Takım",
    "Beray Taş İşlemeli Takım": "Beray Taş İşlemeli Takım",
    "Eylül Düğmeli Gömlek": "Eylül Düğmeli Gömlek",
    "Çıtçıt Düğmeli Uzun Tunik": "Çıtçıt Düğmeli Uzun Tunik",
    "Organize 3'lü Takım": "Organize 3'lü Takım",
    "Apoletli Trenç": "Apoletli Trenç",
    "Desenli Jile Takım": "Desenli Jile Takım",
    "Saten Etekli Takım": "Saten Etekli Takım",
    "Fermuarlı Etekli Takım": "Fermuarlı Etekli Takım",
    "Boncuklu Etekli Takım": "Boncuklu Etekli Takım",
    "Pekşen Pantolonlu Takım": "Pekşen Pantolonlu Takım",
    "Kampanyalı Etekli Takımlar (3 Model)": "Kampanyalı Etekli Takımlar (3 Model)",
    "Tensel Etekli Takım": "Tensel Etekli Takım",
    "Taş İşlemeli Etekli Takım": "Taş İşlemeli Etekli Takım",
    "Spor Bsn Etekli Takım": "Spor Bsn Etekli Takım",
    "Güpürlü Elbise": "Güpürlü Elbise",
    "Jileli Elbise": "Jileli Elbise",
    "Jileli Bronşlu Elbise": "Jileli Bronşlu Elbise",
    "Bol Paça Pantolon": "Bol Paça Pantolon",
    "İspanyol Paça Dabil Pantolon": "İspanyol Paça Dabil Pantolon",
    "Beyoğlu Taşlı Pantolon": "Beyoğlu Taşlı Pantolon",
    
    // Yorumlar & Güven Rozetleri
    "review_1_text": "Kumaş kalitesi beklediğimden çok daha iyi çıktı. Kargo çok hızlıydı, teşekkürler Cemre Park!",
    "review_2_text": "Takımın duruşu efsane! Özel günümde kurtarıcım oldu. Kesinlikle tavsiye ediyorum.",
    "review_3_text": "Müşteri iletişimi harika. Beden konusunda çok yardımcı oldular, ürün tam üzerime göre.",
    
    "badge_fast_delivery": "Hızlı Teslimat",
    "badge_fast_delivery_desc": "Tüm Türkiye'ye hızlı kargo imkanı",
    "badge_secure_payment": "Güvenli Ödeme",
    "badge_secure_payment_desc": "Kapıda Nakit veya Kredi Kartı",
    "badge_customer_satisfaction": "Müşteri Memnuniyeti",
    "badge_customer_satisfaction_desc": "Yüzlerce mutlu müşteri",
    "badge_whatsapp_support": "WhatsApp Destek",
    "badge_whatsapp_support_desc": "0554 169 89 09",
    
    "menu_clothing": "Giyim",
    "menu_two_piece": "İkili Takımlar",
    "menu_dresses": "Elbiseler",
    "menu_tunics": "Tunikler",
    "menu_trousers": "Pantolonlar",
    
    "menu_outerwear": "Dış Giyim",
    "menu_jacket_blazer": "Ceket & Blazer",
    "menu_trench": "Trençkot",
    "menu_coat_jacket": "Kaban & Mont",
    
    "favorite_added_toast": "Favorilere Eklendi!",
    "favorite_removed_toast": "Favorilerden Çıkarıldı!",
    "favorite_add_title": "Favorilere Ekle",
    "favorite_remove_title": "Favorilerden Çıkar",
    
    "home": "Ana Sayfa",
    "select_size": "Beden Seçiniz",
    "select_color": "Renk Seçiniz",
    
    "favorites": "Favorilerim",
    "no_favorites": "Favori Ürününüz Yok",
    "no_favorites_desc": "Beğendiğiniz ürünleri favorilerinize ekleyerek daha sonra kolayca bulabilirsiniz.",
    
    "Siyah": "Siyah",
    "Bej": "Bej",
    "Haki": "Haki",
    "pembe": "Pembe",
    "yar": "Yar",
    
    "search_placeholder": "Ürün, kategori arayın...",
    "popular_searches": "Popüler Aramalar",
    "search_hijab_dress": "Tesettür Elbise",
    "search_two_piece": "İkili Takım",
    "search_tunic": "Tunik",
    "search_coat": "Kap & Kaban",
    "search_trousers": "Pantolon",
    "search_evening_dress": "Abiye",
    
    "expiry": "SKT (AA/YY)",
    "cvv": "CVV",
    "complete_order": "Siparişi Tamamla",
    "fast_shipping": "Hızlı Teslimat",
    "fast_shipping_desc": "24 saatte kargoda",
    "secure_payment_title": "Güvenli Ödeme",
    "secure_payment_desc": "256-bit SSL koruması",
    "product_specs": "Ürün Özellikleri",
    "fabric": "Kumaş",
    "washing": "Yıkama",
    "made_in": "Üretim Yeri",
    "similar_products": "Benzer Ürünler",
    "fabric_type": "Premium krep kumaş",
    "washing_instruction": "30 derecede hassas yıkama",
    "turkey": "Türkiye",
    "remove": "Kaldır",
    "price_col": "Fiyat",
    "order_success": "Ödeme Başarılı!",
    "order_success_desc": "Siparişiniz başarıyla alınmıştır. Bizi tercih ettiğiniz için teşekkür ederiz.",
    "back_to_home": "Alışverişe Dön",
    "processing": "İşleniyor...",
    "ssl_guarantee": "256-bit SSL sertifikası ile ödemeniz güvence altındadır.",
    "search_now": "Arama Yap",
    "about_us_p1": "Cemre Park olarak, modern tesettür giyimin zarafetini ve kalitesini bir araya getiren özgün tasarımlarımızla yıllardır sektörde öncü olmaktan gurur duyuyoruz.",
    "about_us_p2": "Amacımız, her kadının kendi stilini bulabileceği, günün her saatine uygun, konforlu ve şık seçenekler sunmaktır. Premium kumaş kalitemiz, usta işçiliğimiz ve güncel moda trendlerini yakalayan vizyonumuzla her sezon yepyeni bir heyecanla karşınızdayız.",
    "our_vision_title": "Vizyonumuz",
    "our_vision_desc": "Türkiye'nin ve dünyanın dört bir yanına ulaşan, modaya yön veren en güvenilir tesettür giyim markası olmak.",
    "contact_intro": "Soru, öneri ve görüşleriniz bizim için çok değerli. Bize aşağıdaki kanallardan ulaşabilirsiniz.",
    "email_title": "E-Posta",
    "phone_title": "Telefon",
    "address_title": "Adres",
    "distance_selling_text": "İşbu Sözleşme, Alıcı'nın, Satıcı'ya ait cemrepark.com internet sitesinden elektronik ortamda siparişini yaptığı aşağıda nitelikleri ve satış ücreti belirtilen ürünün satışı ve teslimi ile ilgili olarak yürürlükteki yasal mevzuat gereğince tarafların hak ve yükümlülüklerini saptar.",
    "return_policy_text": "Satın almış olduğunuz ürünleri, kullanılmamış, etiketleri sökülmemiş ve tekrar satılabilir özelliğini kaybetmemiş olması şartıyla 14 gün içerisinde iade edebilir veya değiştirebilirsiniz. İade kargo bedelleri firmamız tarafından karşılanmaktadır.",
    "privacy_policy_text": "Müşterilerimize ait kişisel bilgiler, en yüksek güvenlik standartlarında korunmakta olup, hiçbir şekilde üçüncü şahıslar veya firmalarla paylaşılmaz. SSL sertifikası ile korunan altyapımızda güvenle alışveriş yapabilirsiniz.",
    "reviews_title": "Değerlendirmeler",
    "reviews_count": "({count} Değerlendirme)",
    "write_review": "Yorum Yap",
    "your_rating": "Puanınız",
    "your_comment": "Yorumunuz",
    "comment_placeholder": "Ürün hakkındaki düşüncelerinizi paylaşın...",
    "send_comment": "Yorumu Gönder",
    "sending": "Gönderiliyor...",
    "login_required_review": "Yorum yapmak için giriş yapmalısınız.",
    "login": "Giriş Yap",
    "register": "Kayıt Ol",
    "register_title": "KAYIT OL",
    "name_surname": "Ad Soyad",
    "password_label": "Şifre",
    "password_hint": "En az 6 karakter olmalıdır.",
    "registering": "Kayıt Olunuyor...",
    "already_have_account": "Zaten hesabınız var mı?",
    "register_failed": "Kayıt Başarısız",
    "error_occurred": "Bir hata oluştu.",
    "success": "Başarılı!",
    "register_success_desc": "Kaydınız başarıyla oluşturuldu. Şimdi giriş yapabilirsiniz.",
    "error_title": "Hata",
    "register_error_unexpected": "Kayıt olurken beklenmeyen bir hata oluştu.",
    "login_title": "GİRİŞ YAP",
    "forgot_password": "Şifremi Unuttum",
    "logging_in": "Giriş Yapılıyor...",
    "dont_have_account": "Hesabınız yok mu?",
    "login_failed": "Giriş Başarısız",
    "login_invalid_credentials": "E-posta veya şifre hatalı.",
    "my_account": "Hesabım",
    "no_reviews_yet": "Henüz yorum yapılmamış. İlk yorumu siz yapın!",
    "review_success_title": "Teşekkürler!",
    "review_success_desc": "Yorumunuz başarıyla eklendi.",
    "review_warning_title": "Giriş Yapın",
    "review_warning_desc": "Yorum yapmak için giriş yapmalısınız.",
    "review_warning_selection_title": "Seçim Yapmadınız!",
    "review_warning_selection_desc": "Lütfen beden ve renk seçiniz.",
    "cart_added_title": "Sepete Eklendi!",
    "cart_added_desc": "{name} sepetinize başarıyla eklendi.",
    "user_default_name": "Kullanıcı",
    "logout": "Çıkış Yap",
    "quick_links": "Hızlı Bağlantılar",
    "my_orders": "Siparişlerim",
    "my_favorites": "Favorilerim",
    "account_settings": "Hesap Ayarları",
    "back_to_my_account": "Hesabıma Dön",
    "no_orders_yet": "Henüz hiç siparişiniz bulunmuyor.",
    "start_shopping": "Alışverişe Başla",
    "order_no": "Sipariş #{id}",
    "status_completed": "Tamamlandı",
    "status_cancelled": "İptal Edildi",
    "status_preparing": "Hazırlanıyor",
    "status_shipped": "Kargolandı",
    "status_delivered": "Teslim Edildi"
  },
  EN: {
    "collection": "Collection",
    "add_to_cart": "Add to Cart",
    "cart": "Cart",
    "checkout": "Checkout",
    "total": "Total",
    "subtotal": "Subtotal",
    "shipping": "Shipping",
    "free": "Free",
    "search_placeholder": "Search products...",
    "new_season": "NEW SEASON COLLECTION",
    "free_shipping": "FREE SHIPPING ON ALL ORDERS",
    "discount": "10% OFF YOUR FIRST ORDER",
    "installment": "CASH ON DELIVERY & INSTALLMENTS",
    "size": "Size",
    "color": "Color",
    "price": "Price",
    "details": "Full Details",
    "quick_view": "Quick View",
    "no_products": "No Products Found",
    "clear_filters": "Clear Filters",
    "trending": "Trending Now",
    "view_all": "View All",
    "buy_now": "Buy Now",
    "contact": "Contact",
    "about": "About Us",
    "hero_title": "It will suit you so well! 💫",
    "hero_desc": "Rediscover your style with the trendiest hijab suits, coats, and tunics. Shop safely with Wire Transfer, EFT, or Cash/Credit Card on Delivery.",
    "explore_collection": "Explore Collection",
    "search_results": "Search Results",
    "total_products_listed": "Showing {count} products in total.",
    "filters": "Filters",
    "price_range": "Price Range",
    "remove_filters": "Remove Filters",
    "no_products_found": "No Products Found",
    "no_products_found_desc": "We couldn't find any products matching your criteria.",
    "quick_view_desc": "This product is part of the Cemre Park special collection. It stands out with its eye-catching design and premium fabric quality.",
    "explore": "Explore",
    "corporate": "Corporate",
    "newsletter": "Join Newsletter",
    "sets": "Sets",
    "tunics": "Tunics",
    "jackets": "Jackets",
    "dresses": "Dresses",
    "trousers": "Trousers",
    "discover_collection": "Discover Collection",
    "special_design": "Special Design",
    "premium_coats": "Premium Coat Series",
    "explore_collection_btn": "Explore Collection",
    "minimalist_dresses": "Minimalist Dresses",
    "modern_tunics": "Modern Tunics",
    "testimonials_title": "What Our Customers Say?",
    "modern_tunics": "Modern Tunics",
    "testimonials_title": "What Our Customers Say?",
    "testimonials_desc": "Join hundreds of our happy customers.",
    "collections_menu": "COLLECTIONS",
    "discover_new_season": "DISCOVER OUR NEW SEASON COLLECTION",
    "shopier_store": "Shopier Store",
    "suits_you_well": "It will suit you so well! ✨",
    "add_to_favorites": "ADD TO FAVORITES",
    "remove_from_favorites": "REMOVE FROM FAVORITES",

    // NEW TRANSLATIONS
    "footer_desc": "The address of elegance and quality in modern modest clothing. Reflect your style with our latest coats, tunics, and sets.",
    "explore_menu_new": "New Arrivals",
    "explore_menu_best": "Best Sellers",
    "explore_menu_coats": "Coats & Jackets",
    "explore_menu_accessories": "Accessories",
    "about_us": "About Us",
    "contact_us": "Contact Us",
    "distance_selling": "Distance Selling Contract",
    "return_policy": "Return and Exchange Policy",
    "privacy_policy": "Privacy Policy",
    "newsletter_desc": "Be the first to know about special collections.",
    "email_placeholder": "Your Email Address...",
    "all_rights": "All rights reserved.",
    "secure_payment_infra": "High Security Payment Infrastructure.",
    "secure_100": "100% Secure",
    "subscribe": "Subscribe",
    "thanks": "Thank you!",
    "newsletter_success": "You have successfully subscribed to the newsletter.",
    "vip_family": "JOIN OUR VIP FAMILY",
    "vip_desc": "Be the first to hear about new season collections, special discounts and campaigns.",
    "my_cart": "My Cart",
    "cart_empty": "Your cart is empty.",
    "continue_shopping": "Continue Shopping",
    "product_col": "Product",
    "quantity_col": "Quantity",
    "order_summary": "Order Summary",
    "secure_shopping": "100% Secure Shopping",
    "grand_total": "Grand Total",
    "checkout_title": "Secure Checkout",
    "shipping_info": "Shipping Information",
    "full_name": "Full Name",
    "phone": "Phone",
    "city": "City",
    "district": "District",
    "address": "Full Address",
    "payment_info": "Payment Information",
    "card_number": "Card Number",
    "expiry": "Exp (MM/YY)",
    "cvv": "CVV",
    "complete_order": "Complete Order",
    "fast_shipping": "Fast Shipping",
    "fast_shipping_desc": "Ships in 24 hours",
    "secure_payment_title": "Secure Payment",
    "secure_payment_desc": "256-bit SSL protection",
    "product_specs": "Product Specifications",
    "fabric": "Fabric",
    "washing": "Washing",
    "made_in": "Made In",
    "similar_products": "Similar Products",
    "fabric_type": "Premium crepe fabric",
    "washing_instruction": "Delicate wash at 30 degrees",
    "turkey": "Turkey",
    "remove": "Remove",
    "price_col": "Price",
    "order_success": "Payment Successful!",
    "order_success_desc": "Your order has been received successfully. Thank you for choosing us.",
    "back_to_home": "Back to Home",
    "processing": "Processing...",
    "ssl_guarantee": "Your payment is secured with a 256-bit SSL certificate.",
    "search_now": "Search Now",
    "dfsfs": "DFSFS",
    "Yeni Sezon": "New Season",
    "Tükendi": "Sold Out",
    "Takım": "Suit/Set",
    "Tunik": "Tunic",
    "Gömlek": "Shirt",
    "Ceket": "Jacket",
    "Elbise": "Dress",
    "Pantolon": "Trousers",
    "fsf": "Other",
    "selim": "Selim",
    "Volan Detaylı Etekli Takım": "Ruffle Detail Skirt Set",
    "Büyük Beden Jakarlı Tunik": "Plus Size Jacquard Tunic",
    "Modal Nakışlı Tunik": "Modal Embroidered Tunic",
    "Modal Pantolonlu Takım": "Modal Trouser Set",
    "Beray Taş İşlemeli Takım": "Beray Stone Embroidered Set",
    "Eylül Düğmeli Gömlek": "Eylül Buttoned Shirt",
    "Çıtçıt Düğmeli Uzun Tunik": "Snap Button Long Tunic",
    "Organize 3'lü Takım": "Organza 3-Piece Set",
    "Apoletli Trenç": "Epaulette Trench Coat",
    "Desenli Jile Takım": "Patterned Jumper Set",
    "Saten Etekli Takım": "Satin Skirt Set",
    "Fermuarlı Etekli Takım": "Zippered Skirt Set",
    "Boncuklu Etekli Takım": "Beaded Skirt Set",
    "Pekşen Pantolonlu Takım": "Pekşen Trouser Set",
    "Kampanyalı Etekli Takımlar (3 Model)": "Discounted Skirt Sets (3 Models)",
    "Tensel Etekli Takım": "Tencel Skirt Set",
    "Taş İşlemeli Etekli Takım": "Stone Embroidered Skirt Set",
    "Spor Bsn Etekli Takım": "Sport BSN Skirt Set",
    "Güpürlü Elbise": "Guipure Dress",
    "Jileli Elbise": "Jumper Dress",
    "Jileli Bronşlu Elbise": "Brooch Jumper Dress",
    "Bol Paça Pantolon": "Wide Leg Trousers",
    "İspanyol Paça Dabil Pantolon": "Flared Double Trousers",
    "Beyoğlu Taşlı Pantolon": "Beyoglu Stone Trousers",
    
    "review_1_text": "The fabric quality was much better than I expected. Shipping was very fast, thanks Cemre Park!",
    "review_2_text": "The stance of the suit is legendary! It was my savior on my special day. I definitely recommend it.",
    "review_3_text": "Great customer communication. They were very helpful with the sizing, the product fits me perfectly.",
    
    "badge_fast_delivery": "Fast Delivery",
    "badge_fast_delivery_desc": "Fast shipping opportunity to all over Turkey",
    "badge_secure_payment": "Secure Payment",
    "badge_secure_payment_desc": "Cash on Delivery or Credit Card",
    "badge_customer_satisfaction": "Customer Satisfaction",
    "badge_customer_satisfaction_desc": "Hundreds of happy customers",
    "badge_whatsapp_support": "WhatsApp Support",
    "badge_whatsapp_support_desc": "0554 169 89 09",
    
    "menu_clothing": "Clothing",
    "menu_two_piece": "Two-Piece Sets",
    "menu_dresses": "Dresses",
    "menu_tunics": "Tunics",
    "menu_trousers": "Trousers",
    
    "menu_outerwear": "Outerwear",
    "menu_jacket_blazer": "Jacket & Blazer",
    "menu_trench": "Trench Coat",
    "menu_coat_jacket": "Coat & Jacket",
    
    "favorite_added_toast": "Added to Favorites!",
    "favorite_removed_toast": "Removed from Favorites!",
    "favorite_add_title": "Add to Favorites",
    "favorite_remove_title": "Remove from Favorites",
    
    "home": "Home",
    "select_size": "Select Size",
    "select_color": "Select Color",
    
    "favorites": "My Favorites",
    "no_favorites": "No Favorite Products",
    "no_favorites_desc": "You can easily find the products you like later by adding them to your favorites.",
    
    "Siyah": "Black",
    "Bej": "Beige",
    "Haki": "Khaki",
    "pembe": "Pink",
    "yar": "Yar",
    
    "search_placeholder": "Search product, category...",
    "popular_searches": "Popular Searches",
    "search_hijab_dress": "Hijab Dress",
    "search_two_piece": "Two-Piece Set",
    "search_tunic": "Tunic",
    "search_coat": "Coat & Jacket",
    "search_trousers": "Trousers",
    "search_evening_dress": "Evening Dress",
    "about_us_p1": "As Cemre Park, we are proud to be a pioneer in the sector for years with our unique designs that bring together the elegance and quality of modern modest clothing.",
    "about_us_p2": "Our goal is to offer comfortable and stylish options suitable for every hour of the day, where every woman can find her own style. We are here with a brand new excitement every season with our premium fabric quality, master craftsmanship and our vision that catches the current fashion trends.",
    "our_vision_title": "Our Vision",
    "our_vision_desc": "To be the most reliable modest clothing brand that reaches all over Turkey and the world, and shapes fashion.",
    "contact_intro": "Your questions, suggestions and feedback are very valuable to us. You can reach us through the channels below.",
    "email_title": "Email",
    "phone_title": "Phone",
    "address_title": "Address",
    "distance_selling_text": "This Agreement determines the rights and obligations of the parties in accordance with the current legal legislation regarding the sale and delivery of the product, whose qualities and sales price are specified below, ordered electronically by the Buyer from the Seller's website.",
    "return_policy_text": "You can return or exchange the products you have purchased within 14 days, provided that they have not been used, their labels have not been removed and they have not lost their resalability feature. Return shipping costs are covered by our company.",
    "privacy_policy_text": "Personal information belonging to our customers is protected at the highest security standards and is in no way shared with third parties or companies. You can shop safely in our infrastructure protected by an SSL certificate.",
    "reviews_title": "Reviews",
    "reviews_count": "({count} Reviews)",
    "write_review": "Write a Review",
    "your_rating": "Your Rating",
    "your_comment": "Your Comment",
    "comment_placeholder": "Share your thoughts about the product...",
    "send_comment": "Submit Comment",
    "sending": "Sending...",
    "login_required_review": "You must log in to post a review.",
    "login": "Log In",
    "register": "Register",
    "register_title": "REGISTER",
    "name_surname": "Full Name",
    "password_label": "Password",
    "password_hint": "Must be at least 6 characters.",
    "registering": "Registering...",
    "already_have_account": "Already have an account?",
    "register_failed": "Registration Failed",
    "error_occurred": "An error occurred.",
    "success": "Success!",
    "register_success_desc": "Your registration has been successfully created. You can now log in.",
    "error_title": "Error",
    "register_error_unexpected": "An unexpected error occurred during registration.",
    "login_title": "LOG IN",
    "forgot_password": "Forgot Password?",
    "logging_in": "Logging In...",
    "dont_have_account": "Don't have an account?",
    "login_failed": "Login Failed",
    "login_invalid_credentials": "Invalid email or password.",
    "my_account": "My Account",
    "no_reviews_yet": "No reviews yet. Be the first to review!",
    "review_success_title": "Thank you!",
    "review_success_desc": "Your review has been successfully added.",
    "review_warning_title": "Log In",
    "review_warning_desc": "You must log in to leave a review.",
    "review_warning_selection_title": "No Selection Made!",
    "review_warning_selection_desc": "Please select size and color.",
    "cart_added_title": "Added to Cart!",
    "cart_added_desc": "{name} has been successfully added to your cart.",
    "user_default_name": "User",
    "logout": "Sign Out",
    "quick_links": "Quick Links",
    "my_orders": "My Orders",
    "my_favorites": "My Favorites",
    "account_settings": "Account Settings",
    "back_to_my_account": "Back to My Account",
    "no_orders_yet": "You don't have any orders yet.",
    "start_shopping": "Start Shopping",
    "order_no": "Order #{id}",
    "status_completed": "Completed",
    "status_cancelled": "Cancelled",
    "status_preparing": "Preparing",
    "status_shipped": "Shipped",
    "status_delivered": "Delivered"
  },
  AR: {
    "collection": "مجموعة",
    "add_to_cart": "أضف إلى السلة",
    "cart": "عربة التسوق",
    "checkout": "الدفع",
    "total": "المجموع",
    "subtotal": "المجموع الفرعي",
    "shipping": "الشحن",
    "free": "مجانًا",
    "search_placeholder": "البحث عن المنتجات...",
    "new_season": "مجموعة الموسم الجديد",
    "free_shipping": "شحن مجاني على جميع الطلبات",
    "discount": "خصم 10٪ على طلبك الأول",
    "installment": "الدفع نقدًا عند الاستلام وأقساط",
    "size": "مقاس",
    "color": "لون",
    "price": "السعر",
    "details": "التفاصيل",
    "quick_view": "نظرة سريعة",
    "no_products": "لا توجد منتجات",
    "clear_filters": "مسح الفلاتر",
    "trending": "الرائجة الآن",
    "view_all": "عرض الكل",
    "buy_now": "اشتر الآن",
    "contact": "اتصل بنا",
    "about": "معلومات عنا",
    "hero_title": "سوف يناسبك تمامًا! 💫",
    "hero_desc": "أعيدي اكتشاف أسلوبك مع أحدث بدلات المحجبات، المعاطف والسترات. تسوقي بأمان.",
    "explore_collection": "استكشف المجموعة",
    "search_results": "نتائج البحث",
    "total_products_listed": "عرض {count} منتجات",
    "filters": "مرشحات",
    "price_range": "نطاق السعر",
    "remove_filters": "إزالة المرشحات",
    "no_products_found": "لم يتم العثور على أي منتج",
    "no_products_found_desc": "لم نتمكن من العثور على أي منتجات.",
    "quick_view_desc": "هذا المنتج جزء من مجموعة سيمري بارك الخاصة.",
    "explore": "اكتشف",
    "corporate": "الشركات",
    "newsletter": "النشرة الإخبارية",
    "sets": "أطقم",
    "tunics": "ستر",
    "jackets": "جاكيتات",
    "dresses": "فساتين",
    "trousers": "بنطلون",
    "discover_collection": "استكشف المجموعة",
    "special_design": "تصميم خاص",
    "premium_coats": "سلسلة معاطف فاخرة",
    "explore_collection_btn": "تصفح المجموعة",
    "minimalist_dresses": "فساتين بسيطة",
    "modern_tunics": "ستر حديثة",
    "testimonials_title": "ماذا يقول عملاؤنا؟",
    "modern_tunics": "ستر حديثة",
    "testimonials_title": "ماذا يقول عملاؤنا؟",
    "testimonials_desc": "انضم إلى مئات العملاء السعداء.",
    "collections_menu": "مجموعات",
    "discover_new_season": "اكتشف مجموعة الموسم الجديد",
    "shopier_store": "متجر شوبير",
    "suits_you_well": "سوف يناسبك تمامًا! ✨",
    "add_to_favorites": "أضف إلى المفضلة",
    "remove_from_favorites": "إزالة من المفضلة",

    // NEW TRANSLATIONS
    "footer_desc": "عنوان الأناقة والجودة في الملابس المحتشمة الحديثة. اعكسي أسلوبك مع أحدث المعاطف والسترات والأطقم.",
    "explore_menu_new": "القادمون الجدد",
    "explore_menu_best": "الأكثر مبيعا",
    "explore_menu_coats": "معاطف وجاكيتات",
    "explore_menu_accessories": "إكسسوارات",
    "about_us": "معلومات عنا",
    "contact_us": "اتصل بنا",
    "distance_selling": "عقد البيع عن بعد",
    "return_policy": "سياسة الإرجاع والاستبدال",
    "privacy_policy": "سياسة الخصوصية",
    "newsletter_desc": "كن أول من يعرف عن المجموعات الخاصة.",
    "email_placeholder": "عنوان بريدك الإلكتروني...",
    "all_rights": "كل الحقوق محفوظة.",
    "secure_payment_infra": "بنية تحتية عالية الأمان للدفع.",
    "secure_100": "آمن 100%",
    "subscribe": "إشتراك",
    "thanks": "شكراً لك!",
    "newsletter_success": "لقد اشتركت بنجاح في النشرة الإخبارية.",
    "vip_family": "انضم إلى عائلة VIP لدينا",
    "vip_desc": "كن أول من يسمع عن مجموعات الموسم الجديد والخصومات والحملات الخاصة.",
    "my_cart": "عربة التسوق الخاصة بي",
    "cart_empty": "عربة التسوق فارغة.",
    "continue_shopping": "مواصلة التسوق",
    "product_col": "المنتج",
    "quantity_col": "الكمية",
    "order_summary": "ملخص الطلب",
    "secure_shopping": "تسوق آمن 100%",
    "grand_total": "المجموع الإجمالي",
    "checkout_title": "الخروج الآمن",
    "shipping_info": "معلومات الشحن",
    "full_name": "الاسم الكامل",
    "phone": "الهاتف",
    "city": "المدينة",
    "district": "المنطقة",
    "address": "العنوان الكامل",
    "payment_info": "معلومات الدفع",
    "card_number": "رقم البطاقة",
    "expiry": "تاريخ الانتهاء (شهر/سنة)",
    "cvv": "CVV",
    "complete_order": "إكمال الطلب",
    "fast_shipping": "شحن سريع",
    "fast_shipping_desc": "يُشحن خلال 24 ساعة",
    "secure_payment_title": "دفع آمن",
    "secure_payment_desc": "حماية SSL 256 بت",
    "product_specs": "مواصفات المنتج",
    "fabric": "قماش",
    "washing": "غسيل",
    "made_in": "صنع في",
    "similar_products": "منتجات مماثلة",
    "fabric_type": "قماش كريب فاخر",
    "washing_instruction": "غسيل دقيق في درجة حرارة 30",
    "turkey": "تركيا",
    "remove": "إزالة",
    "price_col": "السعر",
    "order_success": "الدفع ناجح!",
    "order_success_desc": "تم استلام طلبك بنجاح. شكرا لاختيارك لنا.",
    "back_to_home": "العودة إلى الرئيسية",
    "processing": "معالجة...",
    "ssl_guarantee": "دفعتك مؤمنة بشهادة SSL 256 بت.",
    "search_now": "ابحث الآن",
    "dfsfs": "DFSFS",
    "Yeni Sezon": "موسم جديد",
    "Tükendi": "نفذت الكمية",
    "Takım": "طقم",
    "Tunik": "تونيك",
    "Gömlek": "قميص",
    "Ceket": "جاكيت",
    "Elbise": "فستان",
    "Pantolon": "بنطلون",
    "fsf": "أخرى",
    "selim": "سليم",
    "Volan Detaylı Etekli Takım": "طقم تنورة بتفاصيل كشكش",
    "Büyük Beden Jakarlı Tunik": "تونيك جاكار مقاس كبير",
    "Modal Nakışlı Tunik": "تونيك مطرز مودال",
    "Modal Pantolonlu Takım": "طقم بنطلون مودال",
    "Beray Taş İşlemeli Takım": "طقم بيريه مطرز بالحجر",
    "Eylül Düğmeli Gömlek": "قميص أيلول بأزرار",
    "Çıtçıt Düğmeli Uzun Tunik": "تونيك طويل بأزرار كبس",
    "Organize 3'lü Takım": "طقم أورجانزا 3 قطع",
    "Apoletli Trenç": "معطف خندق بكتافات",
    "Desenli Jile Takım": "طقم فستان بدون أكمام منقوش",
    "Saten Etekli Takım": "طقم تنورة ساتان",
    "Fermuarlı Etekli Takım": "طقم تنورة بسحاب",
    "Boncuklu Etekli Takım": "طقم تنورة مطرز بالخرز",
    "Pekşen Pantolonlu Takım": "طقم بنطلون بيكشين",
    "Kampanyalı Etekli Takımlar (3 Model)": "أطقم تنانير مخفضة (3 موديلات)",
    "Tensel Etekli Takım": "طقم تنورة تينسيل",
    "Taş İşlemeli Etekli Takım": "طقم تنورة مطرز بالحجر",
    "Spor Bsn Etekli Takım": "طقم تنورة رياضي",
    "Güpürlü Elbise": "فستان غيبور",
    "Jileli Elbise": "فستان بدون أكمام",
    "Jileli Bronşlu Elbise": "فستان بدون أكمام ببروش",
    "Bol Paça Pantolon": "بنطلون واسع الساق",
    "İspanyol Paça Dabil Pantolon": "بنطلون واسع من الأسفل",
    "Beyoğlu Taşlı Pantolon": "بنطلون بيوغلو بحجر",
    
    "review_1_text": "جودة القماش كانت أفضل بكثير مما توقعت. الشحن كان سريعاً جداً، شكراً جمرة بارك!",
    "review_2_text": "مظهر الطقم أسطوري! كان منقذي في يومي الخاص. أوصي به بالتأكيد.",
    "review_3_text": "تواصل رائع مع العملاء. لقد كانوا متعاونين للغاية في تحديد المقاس، المنتج يناسبني تماماً.",
    
    "badge_fast_delivery": "توصيل سريع",
    "badge_fast_delivery_desc": "فرصة شحن سريع لجميع أنحاء تركيا",
    "badge_secure_payment": "دفع آمن",
    "badge_secure_payment_desc": "الدفع نقدًا عند الاستلام أو بالبطاقة الائتمانية",
    "badge_customer_satisfaction": "رضا العملاء",
    "badge_customer_satisfaction_desc": "المئات من العملاء السعداء",
    "badge_whatsapp_support": "دعم واتساب",
    "badge_whatsapp_support_desc": "0554 169 89 09",
    
    "menu_clothing": "ملابس",
    "menu_two_piece": "أطقم قطعتين",
    "menu_dresses": "فساتين",
    "menu_tunics": "تونيكات",
    "menu_trousers": "بنطلونات",
    
    "menu_outerwear": "ملابس خارجية",
    "menu_jacket_blazer": "جاكيت وبليزر",
    "menu_trench": "معطف خندق",
    "menu_coat_jacket": "معاطف وجاكيتات",
    
    "favorite_added_toast": "تمت الإضافة إلى المفضلة!",
    "favorite_removed_toast": "تمت الإزالة من المفضلة!",
    "favorite_add_title": "أضف إلى المفضلة",
    "favorite_remove_title": "إزالة من المفضلة",
    
    "home": "الرئيسية",
    "select_size": "اختر المقاس",
    "select_color": "اختر اللون",
    
    "favorites": "مفضلاتي",
    "no_favorites": "لا توجد منتجات مفضلة",
    "no_favorites_desc": "يمكنك العثور بسهولة على المنتجات التي تعجبك لاحقًا عن طريق إضافتها إلى مفضلاتك.",
    
    "Siyah": "أسود",
    "Bej": "بيج",
    "Haki": "كاكي",
    "pembe": "وردي",
    "yar": "يار",
    
    "search_placeholder": "ابحث عن منتج، فئة...",
    "popular_searches": "عمليات البحث الشائعة",
    "search_hijab_dress": "فستان محجبات",
    "search_two_piece": "طقم قطعتين",
    "search_tunic": "تونيك",
    "search_coat": "معاطف وجواكت",
    "search_trousers": "بنطلونات",
    "search_evening_dress": "فستان سهرة",
    "about_us_p1": "بصفتنا سيمري بارك، نفخر بكوننا روادًا في هذا القطاع لسنوات من خلال تصميماتنا الفريدة التي تجمع بين الأناقة والجودة في الملابس المحتشمة الحديثة.",
    "about_us_p2": "هدفنا هو تقديم خيارات مريحة وأنيقة مناسبة لكل ساعة من اليوم، حيث يمكن لكل امرأة أن تجد أسلوبها الخاص. نحن هنا بحماس جديد كل موسم بجودة الأقمشة الفاخرة لدينا والحرفية المتقنة ورؤيتنا التي تواكب أحدث اتجاهات الموضة.",
    "our_vision_title": "رؤيتنا",
    "our_vision_desc": "أن نكون العلامة التجارية الأكثر موثوقية للملابس المحتشمة التي تصل إلى جميع أنحاء تركيا والعالم، وتشكل الموضة.",
    "contact_intro": "أسئلتك واقتراحاتك وملاحظاتك قيمة جدا بالنسبة لنا. يمكنك التواصل معنا عبر القنوات أدناه.",
    "email_title": "البريد الإلكتروني",
    "phone_title": "هاتف",
    "address_title": "عنوان",
    "distance_selling_text": "تحدد هذه الاتفاقية حقوق التزامات الأطراف وفقا للتشريعات القانونية الحالية فيما يتعلق ببيع وتسليم المنتج، الذي تم تحديد صفاته وسعر بيعه أدناه، والذي طلبه المشتري إلكترونيا من موقع البائع.",
    "return_policy_text": "يمكنك إرجاع أو استبدال المنتجات التي اشتريتها في غضون 14 يوما، بشرط ألا تكون قد استخدمت، ولم تتم إزالة ملصقاتها ولم تفقد ميزة قابليتها لإعادة البيع. يتم تغطية تكاليف شحن الإرجاع من قبل شركتنا.",
    "privacy_policy_text": "تتم حماية المعلومات الشخصية الخاصة بعملائنا بأعلى معايير الأمان ولا تتم مشاركتها بأي حال من الأحوال مع أطراف ثالثة أو شركات. يمكنك التسوق بأمان في بنيتنا التحتية المحمية بشهادة SSL.",
    "reviews_title": "تقييمات",
    "reviews_count": "({count} تقييمات)",
    "write_review": "أضف تقييم",
    "your_rating": "تقييمك",
    "your_comment": "تعليقك",
    "comment_placeholder": "شاركنا برأيك عن المنتج...",
    "send_comment": "إرسال التعليق",
    "sending": "جاري الإرسال...",
    "login_required_review": "يجب تسجيل الدخول لإضافة تقييم.",
    "login": "تسجيل الدخول",
    "register": "إنشاء حساب",
    "register_title": "إنشاء حساب",
    "name_surname": "الاسم الكامل",
    "password_label": "كلمة المرور",
    "password_hint": "يجب أن تكون 6 أحرف على الأقل.",
    "registering": "جاري التسجيل...",
    "already_have_account": "هل لديك حساب بالفعل؟",
    "register_failed": "فشل التسجيل",
    "error_occurred": "حدث خطأ ما.",
    "success": "نجاح!",
    "register_success_desc": "تم إنشاء حسابك بنجاح. يمكنك الآن تسجيل الدخول.",
    "error_title": "خطأ",
    "register_error_unexpected": "حدث خطأ غير متوقع أثناء التسجيل.",
    "login_title": "تسجيل الدخول",
    "forgot_password": "هل نسيت كلمة المرور؟",
    "logging_in": "جاري تسجيل الدخول...",
    "dont_have_account": "ليس لديك حساب؟",
    "login_failed": "فشل تسجيل الدخول",
    "login_invalid_credentials": "البريد الإلكتروني أو كلمة المرور غير صحيحة.",
    "my_account": "حسابي",
    "no_reviews_yet": "لا توجد تقييمات بعد. كن أول من يكتب تقييماً!",
    "review_success_title": "شكراً لك!",
    "review_success_desc": "تمت إضافة تعليقك بنجاح.",
    "review_warning_title": "تسجيل الدخول",
    "review_warning_desc": "يجب تسجيل الدخول لكتابة تعليق.",
    "review_warning_selection_title": "لم يتم الاختيار!",
    "review_warning_selection_desc": "يرجى تحديد المقاس واللون.",
    "cart_added_title": "تمت الإضافة إلى السلة!",
    "cart_added_desc": "تمت إضافة {name} إلى سلتك بنجاح.",
    "user_default_name": "مستخدم",
    "logout": "تسجيل الخروج",
    "quick_links": "روابط سريعة",
    "my_orders": "طلباتي",
    "my_favorites": "مفضلتي",
    "account_settings": "إعدادات الحساب",
    "back_to_my_account": "العودة إلى حسابي",
    "no_orders_yet": "ليس لديك أي طلبات بعد.",
    "start_shopping": "ابدأ التسوق",
    "order_no": "طلب #{id}",
    "status_completed": "مكتمل",
    "status_cancelled": "ملغي",
    "status_preparing": "قيد التحضير",
    "status_shipped": "تم الشحن",
    "status_delivered": "تم التوصيل"
  }
};
