  ...
  return (
    <div className="max-w-4xl mx-auto px-4 relative z-10">
      <div className="mb-8">
        <Link
          href="/hesabim"
          className="text-gray-600 dark:text-gray-400 hover:text-neon-pink dark:hover:text-neon-pink transition-colors text-sm uppercase tracking-wider flex items-center gap-2 font-medium"
        >
      ...
      <div className="glass-panel p-8 clip-angled">
        {orders.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-600 dark:text-gray-400 mb-6 font-medium">{t("no_orders_yet")}</p>
            <Link
              href="/"
              className="inline-block bg-neon-pink text-white font-bold py-3 px-8 uppercase tracking-widest hover:bg-gray-900 hover:text-white dark:hover:bg-white dark:hover:text-black transition-colors clip-angled cursor-pointer"
            >
              {t("start_shopping")}
            </Link>
          </div>
          ...
          <div className="space-y-6">
            {orders.map((order) => (
              <div
                key={order.id}
                className="border border-black/[0.08] dark:border-white/10 bg-black/[0.03] dark:bg-black/50 p-6 clip-angled flex flex-col md:flex-row justify-between items-start md:items-center gap-4"
              >
                <div>
                  <p className="text-gray-900 dark:text-white font-bold text-lg mb-1">
                    {t("order_no", { id: order.id })}
                  </p>
                  <p className="text-gray-600 dark:text-gray-400 text-sm font-medium">{getLocalDateString(order.createdAt)}</p>
                </div>
