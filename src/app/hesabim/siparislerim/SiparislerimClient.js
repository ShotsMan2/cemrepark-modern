  ...
  return (
    <div className="max-w-4xl mx-auto px-4 relative z-10">
      <div className="mb-8">
        <Link
          href="/hesabim"
          className="text-gray-400 hover:text-neon-pink transition-colors text-sm uppercase tracking-wider flex items-center gap-2"
        >
      ...
      <div className="glass-panel p-8 clip-angled bg-black/40 border border-white/10 backdrop-blur-md">
        {orders.length === 0 ? (
          <div className="text-center py-12">
            ...
            <Link
              href="/"
              className="inline-block bg-neon-pink text-white font-bold py-3 px-8 uppercase tracking-widest hover:bg-white hover:text-black transition-colors clip-angled"
            >
              {t("start_shopping")}
            </Link>
          </div>
          ...
          <div className="space-y-6">
            {orders.map((order) => (
              <div
                key={order.id}
                className="border border-white/10 bg-black/50 p-6 clip-angled flex flex-col md:flex-row justify-between items-start md:items-center gap-4"
              >
                <div>
