    ...
    <div className="min-h-screen pt-32 pb-24 bg-gray-50 dark:bg-transparent relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="flex flex-col items-center mb-16" data-aos="fade-down">
          ...
          {cartItems.length > 0 && <p className="text-foreground/60 mt-4 font-bold tracking-widest uppercase text-sm">{cartItems.length} Ürün</p>}
        </div>
        ...
        {cartItems.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20" data-aos="zoom-in">
            ...
            <h2 className="text-3xl font-black text-foreground mb-4 tracking-wider">{t('cart_empty')}</h2>
            <p className="text-foreground/60 mb-10 max-w-md text-center">{t('no_products_found_desc')}</p>
            <Link
              href="/search"
                  ...
                  <div key={`${item.id}-${item.beden}-${item.renk}-${index}`} data-aos="fade-right" data-aos-delay={index * 100} className="bg-white/40 dark:bg-zinc-900/40 backdrop-blur-3xl p-5 rounded-[2.5rem] shadow-xl border border-white/50 dark:border-white/10 flex flex-col sm:flex-row gap-6 items-center group hover:shadow-2xl hover:shadow-neon-pink/20 transition-all duration-500 transform hover:-translate-y-2 relative overflow-hidden">
                    ...
                    <div className="flex-1 flex flex-col sm:flex-row justify-between w-full relative z-10">
                      <div className="flex flex-col justify-center gap-3">
                        <Link href={`/urundetay/${item.id}`} className="text-xl font-black text-foreground group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-neon-pink group-hover:to-holo-gold transition-all duration-300 line-clamp-2">
                          {t(item.ad)}
                        </Link>
                        <div className="flex flex-wrap items-center gap-3">
                          <span className="bg-white/80 dark:bg-zinc-800/80 backdrop-blur-sm text-foreground/80 px-4 py-1.5 rounded-xl text-xs font-black tracking-wider border border-gray-100 dark:border-zinc-700 shadow-sm uppercase">Beden: {item.beden}</span>
                          <span className="bg-white/80 dark:bg-zinc-800/80 backdrop-blur-sm text-foreground/80 px-4 py-1.5 rounded-xl text-xs font-black tracking-wider border border-gray-100 dark:border-zinc-700 shadow-sm uppercase">Renk: {t(item.renk)}</span>
                        </div>
                      ...
                      <div className="flex flex-row sm:flex-col items-center justify-between sm:justify-center gap-6 mt-6 sm:mt-0">
                        <div className="flex items-center bg-white/80 dark:bg-zinc-950/80 backdrop-blur-xl rounded-full border border-glass-border p-1.5 shadow-sm">
                          <button onClick={() => updateQuantity(item.id, item.beden, item.renk, item.quantity - 1)} className="w-10 h-10 rounded-full flex items-center justify-center text-foreground/70 hover:bg-gray-100 dark:hover:bg-zinc-800 hover:text-neon-pink transition-all font-black active:scale-90 text-lg">
                            -
                          </button>
                        ...
                        <button onClick={() => handleRemove(item)} className="w-12 h-12 rounded-full bg-red-50/80 dark:bg-red-500/10 text-red-500 flex items-center justify-center hover:bg-red-500 hover:text-foreground transition-all duration-300 shadow-sm hover:shadow-lg hover:shadow-red-500/30 active:scale-90 group/btn" title={t('remove')}>
            ...
            <div className="w-full lg:w-1/3" data-aos="fade-left" data-aos-delay="200">
              <div className="bg-white/40 dark:bg-zinc-900/40 backdrop-blur-3xl p-10 rounded-[3rem] shadow-2xl border border-white/50 dark:border-white/10 sticky top-32 relative overflow-hidden group">
                ...
                <div className="flex flex-col gap-2 mb-10 relative z-10">
                  <span className="text-foreground/60 font-black text-xs uppercase tracking-[0.3em]">{t('total')}</span>
