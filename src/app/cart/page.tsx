    ...
    <div className="min-h-screen pt-32 pb-24 bg-gray-50 dark:bg-transparent relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="flex flex-col items-center mb-16" data-aos="fade-down">
          ...
          {cartItems.length > 0 && <p className="text-gray-500 dark:text-gray-400 mt-4 font-bold tracking-widest uppercase text-sm">{cartItems.length} Ürün</p>}
        </div>
        ...
        {cartItems.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20" data-aos="zoom-in">
            <div className="w-40 h-40 bg-white/40 dark:bg-zinc-900/40 backdrop-blur-3xl rounded-full shadow-2xl flex items-center justify-center mb-10 border border-white/50 dark:border-white/10 relative overflow-hidden group">
              ...
              <svg className="w-16 h-16 text-neon-pink relative z-10 group-hover:scale-125 transition-transform duration-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path>
              </svg>
            </div>
            <h2 className="text-3xl font-black text-gray-800 dark:text-white mb-4 tracking-wider">{t('cart_empty')}</h2>
            <p className="text-gray-500 dark:text-gray-400 mb-10 max-w-md text-center">{t('no_products_found_desc')}</p>
            <Link
              href="/search"
          ...
          <div className="flex flex-col lg:flex-row gap-12">
            <div className="w-full lg:w-2/3 space-y-6">
              {cartItems.map((item: any, index: number) => {
                ...
                return (
                  <div key={`${item.id}-${item.beden}-${item.renk}-${index}`} data-aos="fade-right" data-aos-delay={index * 100} className="bg-white/40 dark:bg-zinc-900/40 backdrop-blur-3xl p-5 rounded-[2.5rem] shadow-xl border border-white/50 dark:border-white/10 flex flex-col sm:flex-row gap-6 items-center group hover:shadow-2xl hover:shadow-neon-pink/20 transition-all duration-500 transform hover:-translate-y-2 relative overflow-hidden">
                    ...
                    <Link href={`/urundetay/${item.id}`} className="relative w-36 h-48 rounded-2xl overflow-hidden shrink-0 shadow-inner z-10">
                      <Image src={getValidImageUrl(images[0])} alt={item.ad} fill className="object-cover group-hover:scale-110 transition-transform duration-700" sizes="144px" />
                    ...
                    <div className="flex-1 flex flex-col sm:flex-row justify-between w-full relative z-10">
                      <div className="flex flex-col justify-center gap-3">
                        <Link href={`/urundetay/${item.id}`} className="text-xl font-black text-gray-900 dark:text-white group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-neon-pink group-hover:to-holo-gold transition-all duration-300 line-clamp-2">
                          {t(item.ad)}
                        </Link>
                        <div className="flex flex-wrap items-center gap-3">
                          <span className="bg-white/80 dark:bg-zinc-800/80 backdrop-blur-sm text-gray-700 dark:text-gray-300 px-4 py-1.5 rounded-xl text-xs font-black tracking-wider border border-gray-100 dark:border-zinc-700 shadow-sm uppercase">Beden: {item.beden}</span>
                          <span className="bg-white/80 dark:bg-zinc-800/80 backdrop-blur-sm text-gray-700 dark:text-gray-300 px-4 py-1.5 rounded-xl text-xs font-black tracking-wider border border-gray-100 dark:border-zinc-700 shadow-sm uppercase">Renk: {t(item.renk)}</span>
                        </div>
                      ...
                      <div className="flex flex-row sm:flex-col items-center justify-between sm:justify-center gap-6 mt-6 sm:mt-0">
                        <div className="flex items-center bg-white/80 dark:bg-zinc-950/80 backdrop-blur-xl rounded-full border border-gray-200 dark:border-white/10 p-1.5 shadow-sm">
                          <button onClick={() => updateQuantity(item.id, item.beden, item.renk, item.quantity - 1)} className="w-10 h-10 rounded-full flex items-center justify-center text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-zinc-800 hover:text-neon-pink transition-all font-black active:scale-90 text-lg">
                            -
                          </button>
                        ...
                        <button onClick={() => handleRemove(item)} className="w-12 h-12 rounded-full bg-red-50/80 dark:bg-red-500/10 text-red-500 flex items-center justify-center hover:bg-red-500 hover:text-white transition-all duration-300 shadow-sm hover:shadow-lg hover:shadow-red-500/30 active:scale-90 group/btn" title={t('remove')}>
              ...
              <div className="flex justify-end mt-8" data-aos="fade-up">
                <button onClick={handleClear} className="text-xs font-black tracking-widest uppercase text-gray-500 hover:text-red-500 transition-colors flex items-center gap-2 px-6 py-3 rounded-full hover:bg-red-50 dark:hover:bg-red-500/10 active:scale-95 group">
            ...
            <div className="w-full lg:w-1/3" data-aos="fade-left" data-aos-delay="200">
              <div className="bg-white/40 dark:bg-zinc-900/40 backdrop-blur-3xl p-10 rounded-[3rem] shadow-2xl border border-white/50 dark:border-white/10 sticky top-32 relative overflow-hidden group">
                ...
                <h3 className="text-2xl font-black text-gray-900 dark:text-white mb-8 uppercase tracking-[0.2em] relative z-10 flex items-center gap-3">
                  Sipariş Özeti
                </h3>
                ...
                <div className="space-y-4 text-sm font-bold text-gray-600 dark:text-gray-300 relative z-10 mb-8">
                  <div className="flex justify-between items-center bg-white/50 dark:bg-zinc-800/50 p-5 rounded-2xl border border-white/20 dark:border-white/5 shadow-inner">
                    ...
                    <span className="text-gray-900 dark:text-white text-lg">{formatPrice(totalAmount)}</span>
                  </div>
                  <div className="flex justify-between items-center bg-white/50 dark:bg-zinc-800/50 p-5 rounded-2xl border border-white/20 dark:border-white/5 shadow-inner">
                ...
                <div className="flex flex-col gap-2 mb-10 relative z-10">
                  <span className="text-gray-500 dark:text-gray-400 font-black text-xs uppercase tracking-[0.3em]">{t('total')}</span>
                  <span className="text-gray-900 dark:text-white font-black text-5xl text-transparent bg-clip-text bg-gradient-to-r from-neon-pink to-holo-gold tracking-tighter">
                ...
                <button
                  ...
                  className="w-full bg-gradient-to-r from-gray-900 to-black dark:from-white dark:to-gray-200 text-white dark:text-black py-6 rounded-2xl font-black text-lg uppercase tracking-[0.2em] shadow-xl hover:shadow-2xl hover:shadow-neon-pink/20 transition-all duration-500 transform hover:-translate-y-1 active:scale-95 flex items-center justify-center gap-4 group/btn relative z-10"
                >
                  {t('checkout')}
                  <svg className="w-6 h-6 group-hover/btn:translate-x-2 transition-transform duration-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 8l4 4m0 0l-4 4m4-4H3"></path></svg>
                </button>
              </div>
            </div>
          </div>
