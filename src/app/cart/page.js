  ...
  return (
    <div className="min-h-[70vh] pt-24 pb-12 bg-background relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <h1 className="text-3xl md:text-5xl font-black mb-12 text-glow-pink uppercase tracking-widest text-center">
          {t("my_cart")}
        </h1>
        ...
        {cartItems.length === 0 ? (
          <div className="text-center py-20" data-aos="fade-up">
            ...
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">{t("cart_empty")}</h2>
            <p className="text-gray-600 dark:text-gray-400 mb-8">{t("no_products_found_desc")}</p>
            <Link
              href="/search"
              className="bg-black dark:bg-white text-white dark:text-black px-8 py-3 uppercase tracking-widest text-sm font-bold clip-angled hover:bg-neon-pink dark:hover:bg-neon-pink hover:text-white dark:hover:text-white transition-colors"
            >
                    ...
                    >
                      ...
                      <div className="col-span-1 md:col-span-5 flex gap-4 items-center">
                        ...
                        <div>
                          <Link
                            href={`/urundetay/${item.id}`}
                            className="text-lg font-bold text-gray-900 dark:text-white hover:text-neon-pink dark:hover:text-neon-pink transition-colors mb-1 block"
                          >
                            {t(item.ad)}
                          </Link>
            ...
            <div className="w-full lg:w-1/3">
              <div className="glass-panel clip-angled p-8 sticky top-32">
                ...
                <div className="flex justify-between items-center mb-8">
                  <span className="text-gray-900 dark:text-white font-bold text-lg">{t("total")}</span>
                  <span className="text-gray-900 dark:text-white font-bold text-lg">{formatPrice(totalAmount)}</span>
                </div>
                ...
                <button
                  ...
                  className="block w-full bg-holo-gold text-black text-center py-4 font-black uppercase tracking-widest hover:bg-black dark:hover:bg-white hover:text-white dark:hover:text-black transition-colors clip-angled shadow-[0_0_20px_rgba(255,215,0,0.3)]"
                >
                  {t("checkout")}
                </button>
                ...
                <div className="mt-6 flex justify-center gap-3">
                  {/* Visa */}
                  <div className="w-14 h-8 bg-white border border-black/10 dark:border-white/10 rounded flex items-center justify-center shadow">
                    <span className="text-[#1434CB] font-black italic text-sm tracking-tighter">
                      VISA
                    </span>
                  </div>
                  {/* Mastercard */}
                  <div className="w-14 h-8 bg-white dark:bg-[#1a1a1a] border border-black/10 dark:border-white/10 rounded flex items-center justify-center relative overflow-hidden shadow">
