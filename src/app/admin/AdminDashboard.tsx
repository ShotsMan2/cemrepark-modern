...
export default function AdminDashboard({ onLogout }: { onLogout: () => void }) {
  ...
  const renderView = () => {
    switch (activeTab) {
      case "dashboard":
        return <DashboardView products={products} setActiveTab={setActiveTab} />;
      case "products":
        return (
          <ProductsView
            products={products}
  ...
  const getBreadcrumbTitle = () => {
    const titles = {
      ...
      banners: "Vitrin Yönetimi",
      coupons: "Kampanyalar",
      "ai-support": "AI Destek Asistanı",
      security: "Güvenlik & Loglar",
      ...
      <main className="flex-1 flex flex-col h-screen overflow-y-auto relative z-10">
        ...
        <header className="glass-frosted border-x-0 sticky top-0 z-30 flex items-center justify-between px-4 lg:px-8 py-4 shadow-sm">
          ...
          <div className="flex items-center gap-3 md:gap-5">
            ...
            >
              ...
              <kbd className="hidden lg:inline-block bg-background px-2 py-0.5 rounded text-[10px] text-foreground/50 border border-glass-border font-sans font-bold">
                Ctrl+K
              </kbd>
            </button>
            ...
            <button
              ...
              className="hidden md:block text-foreground/70 hover:text-primary transition-colors focus:outline-none p-2"
            >
              {isFullscreen ? (
                ...
                <div className="absolute right-0 mt-2 w-80 glass-panel p-0 clip-angled border border-glass-border shadow-2xl z-50 overflow-hidden animate-slide-up">
                  ...
                  <div className="max-h-64 overflow-y-auto p-2">
                    {notifications.map((n) => (
                      <div
                        key={n.id}
                        className="p-3 mb-1 rounded-lg hover:bg-primary/10 transition-colors border-l-2 border-transparent hover:border-primary cursor-pointer group"
                      >
                        <p className="text-foreground text-sm font-medium mb-1 group-hover:text-primary transition-colors">
                          {n.text}
                        </p>
        ...
        {showSearch && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-start justify-center pt-[20vh] animate-fade-in px-4">
            <div
              ref={searchRef}
              className="w-full max-w-2xl bg-background/95 border border-glass-border rounded-xl shadow-2xl overflow-hidden"
            >
              <div className="flex items-center border-b border-glass-border px-4">
                ...
                <input
                  ...
                  autoFocus
                />
                <kbd className="bg-foreground/5 px-2 py-1 rounded text-xs text-foreground/50 border border-glass-border ml-2">
                  ESC
                </kbd>
              </div>
              <div className="p-4">
                ...
                <div className="space-y-1">
                  <button
                    ...
                    className="w-full text-left px-3 py-2 rounded-lg hover:bg-primary/10 hover:text-primary transition-colors flex items-center gap-3"
                  >
                  ...
                  <button
                    ...
                    className="w-full text-left px-3 py-2 rounded-lg hover:bg-secondary/10 hover:text-secondary transition-colors flex items-center gap-3"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path
                        strokeLinecap="round"
