...
export default function PagesView() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        Swal.fire("Başarılı", `Sayfa başarıyla ${editingId ? "güncellendi" : "eklendi"}.`, "success");
    ...
    <div className="space-y-8 animate-fade-in text-left">
      <div className="glass-panel p-8 clip-angled border border-white/5 relative overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-gray-400 text-xs font-bold mb-1 uppercase tracking-wider">URL Slug (Kalıcı Bağlantı) *</label>
          ...
          <div className="space-y-4 flex flex-col">
            <div className="flex-1 flex flex-col">
              ...
              <textarea name="content" value={formData.content} onChange={handleInputChange} className="w-full h-full min-h-[120px] bg-black/50 border border-white/10 text-white px-4 py-3 text-sm focus:border-neon-pink outline-none transition-colors resize-none" placeholder="HTML veya düz metin içeriği buraya yazın..." />
            </div>
          </div>
      ...
      <div className="glass-panel p-8 clip-angled border border-white/5">
        ...
        {isLoading ? (
          ...
          <div className="py-12 text-center text-gray-500 border border-white/5 border-dashed bg-black/20 clip-angled"><p>Henüz eklenmiş bir sayfa bulunmuyor.</p></div>
        ) : (
