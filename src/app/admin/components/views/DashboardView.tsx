...
export default function DashboardView({
  ...
  const [activities, setActivities] = useState([
    ...
    },
    {
      id: 2,
      type: "payment",
      text: "Ayşe Y. siparişi teslim edildi",
      time: "1 saat önce",
      color: "green-500",
      icon: "M5 13l4 4L19 7",
    },
    {
      id: 3,
      type: "stock",
      text: "Ürün stoğu güncellendi (Kap)",
      time: "3 saat önce",
      color: "holo-gold",
    ...
    },
    {
      id: 4,
      type: "user",
      text: "Yeni üye kaydı (zeynep@...)",
      time: "5 saat önce",
      color: "purple-500",
      icon: "M12 14c4 0 7 3 7 7H5c0-4 3-7 7-7zm0-2a4 4 0 110-8 4 4 0 010 8z",
    },
    {
      id: 5,
      type: "payment",
      text: "Ödeme onaylandı #1041",
      time: "Dün",
      color: "green-500",
      icon: "M5 13l4 4L19 7",
    },
    ...
    const fetchRecentMessages = async () => {
      try {
        ...
        if (res.ok) {
          ...
          const messageActivities = messagesData.slice(0, 3).map((msg, i) => ({
            id: `msg-${i}`,
            type: "message",
  ...
  // Live Auto-refresh Feed Simulation
  useEffect(() => {
    const interval = setInterval(() => {
      const types = [
        {
          type: "order",
          text: "Yeni sipariş alındı #" + Math.floor(Math.random() * 10000 + 1000),
          color: "neon-pink",
          icon: "M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z",
        },
        {
          type: "user",
          text: "Yeni üye kaydı (" + Math.random().toString(36).substring(7) + "@gmail.com)",
          color: "purple-500",
          icon: "M12 14c4 0 7 3 7 7H5c0-4 3-7 7-7zm0-2a4 4 0 110-8 4 4 0 010 8z",
        },
        {
          type: "stock",
          text: "Stok uyarısı: Ürün #" + Math.floor(Math.random() * 100),
          color: "holo-gold",
          icon: "M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z",
        },
        {
          type: "payment",
          text: "Ödeme başarılı #" + Math.floor(Math.random() * 10000 + 1000),
          color: "green-500",
          icon: "M5 13l4 4L19 7",
        },
      ];
  ...
  const totalProducts =
    stats && typeof stats.products === "number" ? stats.products : safeProducts.length;
  const activeCategories =
    stats && typeof stats.categories === "number"
      ? stats.categories
      : [...new Set(safeProducts.map((p) => p.kategori))].filter(Boolean).length;
  ...
  const catData =
    stats?.categoryData?.length > 0
      ? stats.categoryData
      : transformCategoryPopularity(safeProducts);
  if (catData.length === 0) {
    catData.push({ name: "Genel", value: 100 });
  }
  const totalCategoryValue = catData.reduce((sum, item) => sum + item.value, 0);
  const loginData = stats?.loginStats || [
    { name: "Başarılı", value: 85 },
    { name: "Başarısız", value: 15 },
  ];
  const orderData =
    stats?.orderStats?.length > 0
      ...
      : [
          ...
          { name: "Onaylandı", value: 120 },
          { name: "Hazırlanıyor", value: 80 },
          { name: "Kargolandı", value: 210 },
          { name: "Teslim Edildi", value: 650 },
          { name: "İptal", value: 12 },
        ];
  ...
  const heatmapData = days.map((day) =>
    hours.map((hour) => {
      // create some fake sales pattern (more in afternoon/evening)
  ...
  const KPICard = ({
    ...
    <motion.div
      ...
      className={`glass-card p-4 clip-angled relative overflow-hidden group hover:border-${colorClass} transition-colors bg-gradient-to-br from-white/5 to-transparent`}
    >
      <div
        className={`absolute top-0 right-0 w-32 h-32 bg-${colorClass} opacity-10 rounded-bl-full group-hover:scale-110 transition-transform`}
      ></div>
      <div className="flex justify-between items-start mb-2">
        <p className="text-foreground/60 text-[10px] font-bold uppercase tracking-widest">
          {title}
        </p>
        <div
          className={`flex items-center gap-1 text-[10px] font-bold ${trend === "up" ? "text-green-400" : "text-red-400"}`}
        >
          {trend === "up" ? (
      ...
      <p className={`text-${colorClass} text-xs font-bold`}>{subtext}</p>
    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-foreground/50 text-sm mt-1">
            Platformun anlık özet durumu ve performans metrikleri
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button
            ...
            className="glass-panel px-4 py-2 flex items-center gap-2 hover:bg-neon-pink/20 hover:border-neon-pink transition-all text-xs font-bold text-foreground clip-angled"
          >
            <svg
          ...
          <button
            ...
            className="glass-panel px-4 py-2 flex items-center gap-2 hover:bg-purple-500/20 hover:border-purple-500 transition-all text-xs font-bold text-foreground clip-angled"
          >
            <svg
            ...
            SİPARİŞLER
            <span className="bg-purple-500 text-foreground text-[10px] px-1.5 py-0.5 rounded-full ml-1">
              {orderData.find((o) => o.name === "Beklemede")?.value || 0}
            </span>
          </button>
          <button className="glass-panel px-4 py-2 flex items-center gap-2 hover:bg-holo-gold/20 hover:border-holo-gold transition-all text-xs font-bold text-foreground clip-angled cursor-default">
            <svg
              className="w-4 h-4 text-holo-gold"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            STOK UYARISI
            <span className="bg-holo-gold text-black text-[10px] px-1.5 py-0.5 rounded-full font-black ml-1">
              {safeProducts.filter((p) => p.stok < 5).length}
            </span>
          </button>
          ...
          <div className="relative group">
            ...
            <div className="absolute right-0 top-full mt-2 w-40 glass-panel p-2 flex flex-col gap-1 opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-opacity z-50">
              <button
                ...
                className="text-left px-3 py-2 text-xs text-foreground hover:bg-neon-pink/20 hover:text-neon-pink rounded font-bold transition-colors"
              >
                CSV İndir
              </button>
            </div>
      ...
      >
        ...
        <KPICard
          ...
          colorClass="success"
          colorHex="#22c55e"
          sparklineData={sparklines[1]}
        />
        <KPICard
          title="Ort. Sepet Değeri"
          value={`${avgCartValue} ₺`}
          subtext="Sipariş başına"
          trend="up"
          trendValue="4"
          colorClass="holo-gold"
          colorHex="#ffd700"
          sparklineData={sparklines[2]}
        />
        <KPICard
          title="Dönüşüm Oranı"
          ...
          colorHex="#a855f7"
          sparklineData={sparklines[3]}
        />

        <KPICard
          title="Toplam Ürün"
          value={totalProducts}
          subtext="+12% geçen aya göre"
          trend="up"
          trendValue="12"
          colorClass="neon-pink"
          colorHex="#ff007f"
          sparklineData={sparklines[4]}
        />
        <KPICard
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
        <StockWarning
          items={safeProducts
            .filter((p) => typeof p.stok === "number" && p.stok < 5)
            .map((p) => ({
              id: p.id,
              ad: p.ad,
              resim: p.gorsel || p.resim,
              stok: p.stok,
              renk: p.renk,
              beden: p.beden,
            }))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 glass-panel p-6 md:p-8 clip-angled relative border border-glass-border h-[450px]">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h3 className="text-foreground font-bold uppercase tracking-widest">
                Gelir ve Hedef Analizi
              </h3>
            ...
            <div className="flex gap-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-foreground/50 text-xs font-bold">Gerçekleşen</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 bg-holo-gold rounded-full"></div>
                <span className="text-foreground/50 text-xs font-bold">Hedef</span>
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
        ...
        <div className="glass-panel p-6 md:p-8 clip-angled relative border border-glass-border min-h-[400px]">
          <h3 className="text-foreground font-bold uppercase tracking-widest mb-1">
            Satış Yoğunluk Haritası
          </h3>
          ...
          <div className="w-full overflow-x-auto">
            <div className="min-w-[600px] grid grid-cols-[auto_repeat(24,1fr)] gap-1 mb-2">
              <div />
              {hours.map((h) => (
                <div key={h} className="text-[10px] text-foreground/60 text-center">
                  {h}
                </div>
              ))}
              {days.map((day, i) => (
                <React.Fragment key={day}>
                  <div className="text-[10px] text-foreground/50 pr-2 text-right self-center">
                    {day}
                  </div>
                  {heatmapData[i].map((val, j) => {
                    ...
                    return (
                      <div
                        key={j}
                        className="relative group cursor-pointer w-full aspect-square rounded-sm transition-all hover:scale-125 hover:z-10 hover:rounded"
                        style={{ backgroundColor: `rgba(255, 0, 127, ${colorOp})` }}
                      >
          ...
          <div className="flex items-center justify-end gap-2 mt-4 text-[10px] text-foreground/50">
            Düşük{" "}
            <div className="w-16 h-2 bg-gradient-to-r from-[rgba(255,0,127,0.1)] to-[rgba(255,0,127,1)] rounded"></div>{" "}
            Yüksek
          </div>
        </div>
        ...
        <div className="glass-panel p-6 md:p-8 clip-angled relative border border-glass-border min-h-[400px] flex flex-col justify-center">
          <h3 className="text-foreground font-bold uppercase tracking-widest mb-1 absolute top-6 left-6 md:left-8">
            Müşteri Dönüşüm Hunisi
          </h3>
          <p className="text-foreground/60 text-xs mb-8 absolute top-12 left-6 md:left-8">
            Sitenize gelen ziyaretçilerin satışa dönüşme yolculuğu
          </p>
          ...
          <div className="flex flex-col items-center w-full gap-4 mt-16 px-4">
            {funnelData.map((d, i) => {
              ...
              const dropOff =
                i > 0 ? ((1 - d.value / funnelData[i - 1].value) * 100).toFixed(0) : null;

              return (
                <div key={d.step} className="w-full flex items-center gap-4 relative">
                  <div className="w-20 text-right text-xs font-bold text-foreground/50">
                    {d.step}
                  </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
        <div className="glass-panel p-6 md:p-8 clip-angled relative border border-glass-border h-[400px] flex flex-col">
          <h3 className="text-foreground font-bold uppercase tracking-widest mb-2">
            Kategori Dağılımı
          </h3>
          <p className="text-foreground/60 text-xs mb-6">
            Satışların ürün kategorilerine göre oranları
          </p>
          ...
          <div className="flex justify-center gap-4 mt-4 flex-wrap">
            {catData.map((entry, index) => (
              <div key={entry.name} className="flex items-center gap-2">
                ...
                ></div>
                <span className="text-foreground/50 text-[10px] font-bold uppercase">
                  {entry.name}
                </span>
              </div>
            ))}
