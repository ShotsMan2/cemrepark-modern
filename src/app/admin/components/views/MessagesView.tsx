...
export default function MessagesView() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const enrichedData = data.map((m) => ({
          ...
          status:
            m.status ||
            (Math.random() > 0.5 ? "okunmadi" : Math.random() > 0.5 ? "okundu" : "yanitlandi"),
        }));
  ...
  const handleBulkMarkAsRead = () => {
    if (selectedMessages.length === 0) return;
    setMessages(
      messages.map((m) => (selectedMessages.includes(m.id) ? { ...m, status: "okundu" } : m))
    );
    setSelectedMessages([]);
  ...
  const applyTemplate = (e) => {
    ...
    if (template === "tesekkur")
      text += "Geri bildiriminiz için teşekkür ederiz. Görüşleriniz bizim için çok değerli.\n\n";
    else if (template === "siparis")
      text +=
        "Siparişiniz başarıyla alınmış olup, kargoya verilmek üzere hazırlanmaktadır. Kargo takip numaranız SMS ile iletilecektir.\n\n";
    else if (template === "iade")
      text +=
        "İade talebiniz işleme alınmıştır. Ürünü faturasıyla birlikte anlaşmalı kargo firmamızla ücretsiz olarak gönderebilirsiniz.\n\n";
    else if (template === "urun")
      text += "İlgilendiğiniz ürünle ilgili detaylı bilgi aşağıdadır: \n\n";
  ...
  const handleSendEmail = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMessages(
          messages.map((m) => (m.id === selectedMessage.id ? { ...m, status: "yanitlandi" } : m))
        );
  ...
  const filteredMessages = messages.filter((msg) => {
    const matchesSearch =
      msg.adSoyad?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      msg.ePosta?.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesStatus =
      statusFilter === "Tümü" ||
  ...
  const toggleSelect = (id) => {
    if (selectedMessages.includes(id))
      setSelectedMessages(selectedMessages.filter((i) => i !== id));
      ...
      <div className="glass-panel p-6 clip-angled mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-foreground/50 text-sm mt-1">
            Müşterilerinizden gelen tüm mesajları buradan yönetebilirsiniz.
          </p>
        </div>
        <div className="text-right">
          <span className="text-xs text-foreground/60 uppercase font-bold tracking-wider">
            Ort. Yanıt Süresi
          </span>
      ...
      <div className="glass-panel p-4 clip-angled mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="flex items-center gap-2 w-full md:w-auto">
          <input
            ...
            className="bg-background/50 border border-glass-border text-foreground px-4 py-2 text-sm focus:border-neon-pink outline-none flex-1 md:w-64"
          />
          <input
            type="date"
            value={dateRange.start}
            onChange={(e) => setDateRange({ ...dateRange, start: e.target.value })}
            className="bg-background/50 border border-glass-border text-foreground px-2 py-2 text-sm outline-none"
          />
        </div>
        <div className="flex gap-2 text-xs font-bold uppercase tracking-wider w-full md:w-auto overflow-x-auto">
          {["Tümü", "Okunmadı", "Yanıtlandı"].map((tab) => (
            <button
              ...
              className={`px-4 py-2 whitespace-nowrap transition-colors border ${statusFilter === tab ? "bg-foreground/10 border-white/20 text-foreground" : "border-transparent text-foreground/60 hover:text-foreground"}`}
            >
              {tab}
            </button>
          ))}
        ...
        <div className="bg-neon-pink/20 border border-neon-pink p-3 mb-6 flex items-center justify-between clip-angled animate-fade-in">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="bg-foreground/10 hover:bg-foreground/20 text-foreground px-3 py-1 text-xs font-bold uppercase"
            >
              Okundu İşaretle
            </button>
            <button
              onClick={handleBulkDelete}
              className="bg-red-500/80 hover:bg-red-500 text-foreground px-3 py-1 text-xs font-bold uppercase"
            >
              Sil
            </button>
          </div>
      ...
      {filteredMessages.length === 0 ? (
        ...
        <div className="space-y-4">
          {filteredMessages.map((msg) => (
            <div
              key={msg.id}
              className={`glass-panel p-0 clip-angled border ${msg.status === "okunmadi" ? "border-l-4 border-l-holo-gold border-glass-border bg-holo-gold/5" : "border-glass-border"} transition-all hover:bg-foreground/5 group`}
            >
              <div className="p-4 flex flex-col md:flex-row gap-4 items-start md:items-center">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start mb-1">
                    <div className="flex items-center gap-3">
                      <h4
                        className={`font-bold ${msg.status === "okunmadi" ? "text-foreground" : "text-foreground/70"}`}
                      >
                        {msg.adSoyad}
                      </h4>
                      {msg.status === "okunmadi" && (
                        <span className="bg-holo-gold text-black text-[9px] px-1.5 py-0.5 font-bold uppercase">
                          Yeni
                        </span>
                      )}
                      {msg.status === "yanitlandi" && (
                        <span className="bg-green-500 text-black text-[9px] px-1.5 py-0.5 font-bold uppercase">
                          Yanıtlandı
                        </span>
                      )}
                    </div>
                  ...
                  <a
                    ...
                    className="text-xs text-neon-pink hover:underline inline-block mb-2"
                  >
                    {msg.ePosta}
                  </a>
                  <p
                    className={`text-sm ${msg.status === "okunmadi" ? "text-gray-200 font-medium" : "text-foreground/50"} line-clamp-2`}
                  >
                    {msg.mesaj}
                  </p>
                </div>
                <div className="flex items-center gap-2 md:opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    ...
                    className="bg-foreground/10 hover:bg-neon-pink text-foreground px-4 py-2 text-xs font-bold uppercase clip-angled transition-colors"
                  >
                    Yanıtla
                  </button>
                  <button
                    onClick={() => handleDelete(msg.id)}
                    className="bg-foreground/5 hover:bg-red-500 text-foreground/50 hover:text-foreground p-2 transition-colors"
                  >
                    <svg
                      width={20}
                      height={20}
        ...
        <div className="border-t border-glass-border p-4 flex justify-between items-center text-foreground/50 text-xs uppercase tracking-wider bg-black/40 mt-6 rounded clip-angled">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="px-3 py-1.5 border border-glass-border hover:border-neon-pink hover:text-neon-pink transition-colors disabled:opacity-30 rounded"
            >
              &lt; Önceki
            </button>
            <span className="px-4 py-1.5 bg-background/50 text-foreground font-bold border border-glass-border rounded">
          ...
          <div className="glass-panel p-6 max-w-2xl w-full clip-angled border border-glass-border relative animate-fade-in text-left flex flex-col max-h-[90vh]">
            ...
            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-4">
              ...
              <form id="replyForm" onSubmit={handleSendEmail} className="space-y-4">
                <div className="flex justify-between items-center">
                  <label className="block text-foreground/50 text-xs font-bold uppercase tracking-wider">
                    Konu
                  </label>
                  <select
                    onChange={applyTemplate}
                    className="bg-foreground/10 text-foreground text-xs px-2 py-1 outline-none font-bold uppercase"
                  >
                ...
                <input
                  ...
                  className="w-full bg-background/50 border border-glass-border text-foreground px-4 py-2 text-sm focus:border-neon-pink outline-none transition-colors"
                />

                <div>
                  <label className="block text-foreground/50 text-xs font-bold mb-1 uppercase tracking-wider">
                    Mesajınız
                  </label>
                  <textarea
                    required
                    ...
                    className="w-full bg-background/50 border border-glass-border text-foreground px-4 py-3 text-sm focus:border-neon-pink outline-none transition-colors resize-none font-sans"
                  />
                </div>
              </form>
            </div>
            ...
            <div className="flex gap-4 pt-4 mt-4 border-t border-glass-border">
              <button
                type="button"
                onClick={() => setSelectedMessage(null)}
                className="flex-1 border border-glass-border text-foreground py-3 uppercase tracking-widest text-xs hover:bg-foreground/5 transition-colors clip-angled font-bold"
              >
                İptal
              </button>
              <button
                ...
                form="replyForm"
                disabled={isSending}
                className="flex-1 bg-neon-pink text-foreground py-3 uppercase tracking-widest text-xs hover:bg-white hover:text-black transition-colors clip-angled font-bold disabled:opacity-50"
              >
