...
export default function DashboardView({ products, setActiveTab }) {
  ...
  const [sparklines] = useState([
    generateSparkline(), generateSparkline(), generateSparkline(), generateSparkline(),
    generateSparkline(), generateSparkline(), generateSparkline(), generateSparkline()
  ]);
  ...
  const loginData = stats?.loginStats || [{name: "Başarılı", value: 85}, {name: "Başarısız", value: 15}];
  const orderData = stats?.orderStats?.length > 0 ? stats.orderStats : [
    { name: "Beklemede", value: 45 }, { name: "Onaylandı", value: 120 }, { name: "Hazırlanıyor", value: 80 },
    { name: "Kargolandı", value: 210 }, { name: "Teslim Edildi", value: 650 }, { name: "İptal", value: 12 }
  ];
  
  const topSellingProductsData = stats?.topSellingProducts?.length > 0 ? stats.topSellingProducts : [
    { name: "Premium Kadın Takım Siyah", value: 124 }, { name: "Yeni Sezon Tunik Bej", value: 98 },
    { name: "Klasik Ceket Ekru", value: 75 }, { name: "Günlük Elbise Desenli", value: 42 }
  ];

  const targetData = [
    { name: "Oca", gerceklesen: 12000, hedef: 15000 }, { name: "Şub", gerceklesen: 18000, hedef: 16000 },
    { name: "Mar", gerceklesen: 25000, hedef: 20000 }, { name: "Nis", gerceklesen: 22000, hedef: 25000 },
    { name: "May", gerceklesen: 32000, hedef: 28000 }, { name: "Haz", gerceklesen: Math.max(35000, totalRevenue), hedef: 30000 },
  ];
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="show" className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <KPICard title="Ort. Sepet Değeri" value={`${avgCartValue} ₺`} subtext="Sipariş başına" trend="up" trendValue="4" colorClass="holo-gold" colorHex="#ffd700" sparklineData={sparklines[2]} />
        <KPICard title="Dönüşüm Oranı" value={`%${conversionRate}`} subtext="Ziyaretçi/Satış" trend="down" trendValue="1.2" colorClass="purple-500" colorHex="#a855f7" sparklineData={sparklines[3]} />
        
        <KPICard title="Toplam Ürün" value={totalProducts} subtext="+12% geçen aya göre" trend="up" trendValue="12" colorClass="neon-pink" colorHex="#ff007f" sparklineData={sparklines[4]} />
        <KPICard title="Aktif Kullanıcı" value={activeUsers} subtext="Son 30 gün" trend="up" trendValue="22" colorClass="blue-500" colorHex="#3b82f6" sparklineData={sparklines[5]} />
        <KPICard title="Aktif Kategoriler" value={activeCategories} subtext="Yeni sezon" trend="up" trendValue="5" colorClass="success" colorHex="#22c55e" sparklineData={sparklines[6]} />
                ...
                >
                  <div className={`p-2 rounded-full bg-${act.color}/20 text-${act.color}`}>
                     <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d={act.icon}></path>
                     </svg>
                  </div>
