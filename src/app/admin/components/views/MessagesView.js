  ...
  return (
    <div className="animate-fade-in">
      <div className="glass-panel p-6 clip-angled mb-8">
        ...
        <p className="text-gray-400 text-sm mt-1">İletişim formundan müşterilerinizin gönderdiği mesajları buradan inceleyebilirsiniz.</p>
      </div>
      ...
      {messages.length === 0 ? (
        <div className="glass-panel p-12 clip-angled text-center border border-white/5">
          <div className="w-16 h-16 rounded-full border border-white/10 mx-auto flex items-center justify-center mb-4">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="gray" strokeWidth="1.5"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path><polyline points="22,6 12,13 2,6"></polyline></svg>
          </div>
        ...
        <div className="space-y-6">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-white/10 bg-white/5 text-gray-400 text-xs font-bold uppercase tracking-wider">
                  ...
                  <th className="p-4 text-center pr-6" style={{ width: '120px' }}>İşlem</th>
                </tr>
              </thead>
          ...
          <div className="grid grid-cols-1 gap-4 md:hidden">
            {messages.map((msg) => (
              <div key={msg.id} className="glass-panel p-6 clip-angled border border-white/5 flex flex-col justify-between">
                <div>
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h4 className="text-white font-bold">{msg.adSoyad}</h4>
                      <a href={`mailto:${msg.ePosta}`} className="text-xs text-neon-pink hover:underline block mt-0.5">{msg.ePosta}</a>
                    </div>
                    <span className="text-[10px] text-gray-500 bg-white/5 px-2 py-1 rounded">{msg.tarih}</span>
                  </div>
                  <p className="text-gray-300 text-sm leading-relaxed mb-4 break-words">{msg.mesaj}</p>
                </div>
      ...
      {selectedMessage && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="glass-panel p-8 max-w-lg w-full clip-angled border border-white/10 relative animate-fade-in text-left">
            ...
            <h3 className="text-xl font-bold text-white mb-6 uppercase tracking-wider">E-Posta Gönder</h3>
            
            <form onSubmit={handleSendEmail} className="space-y-4">
              <div>
                <label className="block text-gray-400 text-xs font-bold mb-1 uppercase tracking-wider">Alıcı</label>
                <input 
                  readOnly 
                  type="text" 
              ...
              <div>
                <label className="block text-gray-400 text-xs font-bold mb-1 uppercase tracking-wider">Konu</label>
                <input 
                  required
                  type="text" 
                  value={replySubject}
              ...
              <div>
                <label className="block text-gray-400 text-xs font-bold mb-1 uppercase tracking-wider">Yanıt Mesajınız</label>
                <textarea 
                  required
                  rows="6"
                  value={replyText}
