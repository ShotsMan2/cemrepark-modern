  ...
  return (
    <div className="space-y-8 animate-fade-in text-left">
      <div className="glass-panel p-8 clip-angled border border-glass-border relative overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-2 space-y-4">
            <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="text" name="slug" required value={formData.slug} onChange={handleInputChange} className="w-full bg-background/50 border border-glass-border text-foreground px-4 py-3 text-sm focus:border-neon-pink outline-none transition-colors" placeholder="Örn: hakkimizda"/>
                </div>
            </div>

            <div className="flex flex-col relative">
              <label className="block text-foreground/50 text-xs font-bold mb-1 uppercase tracking-wider">İçerik</label>
              <textarea name="content" value={formData.content} onChange={handleInputChange} className="w-full h-[250px] bg-background/50 border border-glass-border text-foreground px-4 py-3 text-sm focus:border-neon-pink outline-none transition-colors resize-none font-mono" placeholder="HTML veya düz metin içeriği buraya yazın..."/>
              <div className="absolute bottom-2 right-2 flex gap-4 text-xs text-foreground/60 font-mono bg-background/80 px-2 py-1 rounded">
          ...
          <div className="space-y-4">
            <div className="bg-foreground/5 p-4 clip-angled border border-glass-border space-y-4">
                ...
                <div>
                    ...
                    <textarea name="metaDescription" rows={3} value={formData.metaDescription} onChange={handleInputChange} className="w-full bg-background/50 border border-glass-border text-foreground px-3 py-2 text-xs focus:border-holo-gold outline-none resize-none"/>
                </div>
          ...
          <div className="md:col-span-3 pt-4 border-t border-glass-border flex gap-4 items-center">
            <button type="submit" className="bg-neon-pink text-foreground px-8 py-3 uppercase tracking-widest text-xs font-bold hover:bg-white hover:text-black transition-colors clip-angled">
              {editingId ? "Güncelle" : "Sayfayı Ekle"}
            </button>
            <button type="button" onClick={handlePreview} className="border border-neon-pink text-neon-pink px-8 py-3 uppercase tracking-widest text-xs font-bold hover:bg-neon-pink hover:text-foreground transition-colors clip-angled">
              Önizleme
            </button>
            {editingId && (
              <button type="button" onClick={cancelEdit} className="border border-white/20 text-foreground px-8 py-3 uppercase tracking-widest text-xs font-bold hover:bg-foreground/10 transition-colors clip-angled ml-auto">
                İptal
              </button>
            )}
          </div>
      ...
      <div className="glass-panel p-8 clip-angled border border-glass-border">
        <h3 className="text-xl font-bold text-foreground mb-6 uppercase tracking-wider flex items-center gap-3">
        ...
        {isLoading ? (
          ...
          <div className="py-12 text-center text-foreground/60 border border-glass-border border-dashed bg-black/20 clip-angled">
              ...
              <tbody className="divide-y divide-white/5 text-foreground/70">
                {pages.map((page) => (
                  <tr key={page.id} className="hover:bg-foreground/5 transition-colors group">
                    ...
                    <td className="p-4">
                        <span className={`px-2 py-1 text-[10px] font-bold uppercase tracking-wider ${page.status === 'draft' ? 'bg-gray-500/20 text-foreground/50' : 'bg-green-500/20 text-green-400'}`}>
                    ...
                    <td className="p-4 pr-6 text-right">
                      <div className="flex justify-end gap-2">
                        <a href={`/${page.slug}`} target="_blank" className="text-foreground/50 hover:text-foreground p-2 transition-colors focus:outline-none" title="Görüntüle">
                           <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                        </a>
                        <button onClick={() => handleEdit(page)} className="text-foreground/50 hover:text-holo-gold p-2 transition-colors focus:outline-none" title="Düzenle">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                          </svg>
                        </button>
                        <button onClick={() => handleDelete(page.id)} className="text-foreground/50 hover:text-red-500 p-2 transition-colors focus:outline-none" title="Sil">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
