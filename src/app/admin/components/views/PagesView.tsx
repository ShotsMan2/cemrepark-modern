...
export default function PagesView() {
  ...
  const wordCount =
    formData.content.trim() === "" ? 0 : formData.content.trim().split(/\s+/).length;
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          {!editingId && (
            <select
              ...
              className="bg-foreground/10 text-foreground px-4 py-2 text-xs font-bold uppercase tracking-wider outline-none cursor-pointer"
            >
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-2 space-y-4">
            ...
            <div className="flex flex-col relative">
              ...
              <textarea
                ...
                onChange={handleInputChange}
                className="w-full h-[250px] bg-background/50 border border-glass-border text-foreground px-4 py-3 text-sm focus:border-neon-pink outline-none transition-colors resize-none font-mono"
          ...
          <div className="space-y-4">
            <div className="bg-foreground/5 p-4 clip-angled border border-glass-border space-y-4">
              <h4 className="text-holo-gold font-bold uppercase text-xs tracking-wider border-b border-glass-border pb-2">
                Sayfa Ayarları
              </h4>
              ...
              <div>
                <label className="block text-foreground/50 text-[10px] font-bold mb-1 uppercase tracking-wider">
                  SEO Title
                </label>
                <input
                  type="text"
                  ...
                  onChange={handleInputChange}
                  className="w-full bg-background/50 border border-glass-border text-foreground px-3 py-2 text-xs focus:border-holo-gold outline-none"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={handleInputChange}
                  className="w-full bg-background/50 border border-glass-border text-foreground px-3 py-2 text-xs focus:border-holo-gold outline-none resize-none"
                />
              </div>
          ...
          <div className="md:col-span-3 pt-4 border-t border-glass-border flex gap-4 items-center">
            ...
            <button
              ...
              className="border border-neon-pink text-neon-pink px-8 py-3 uppercase tracking-widest text-xs font-bold hover:bg-neon-pink hover:text-foreground transition-colors clip-angled"
            >
              Önizleme
            </button>
            {editingId && (
              <button
                type="button"
                onClick={cancelEdit}
                className="border border-white/20 text-foreground px-8 py-3 uppercase tracking-widest text-xs font-bold hover:bg-foreground/10 transition-colors clip-angled ml-auto"
              >
                İptal
              </button>
            )}
              ...
              <tbody className="divide-y divide-white/5 text-foreground/70">
                {pages.map((page) => (
                  <tr key={page.id} className="hover:bg-foreground/5 transition-colors group">
                    ...
                    <td className="p-4">
                      <span
                        className={`px-2 py-1 text-[10px] font-bold uppercase tracking-wider ${page.status === "draft" ? "bg-gray-500/20 text-foreground/50" : "bg-green-500/20 text-green-400"}`}
                      >
                    ...
                    <td className="p-4 pr-6 text-right">
                      <div className="flex justify-end gap-2">
                        <a
                          href={`/${page.slug}`}
                          target="_blank"
                          className="text-foreground/50 hover:text-foreground p-2 transition-colors focus:outline-none"
                          title="Görüntüle"
                        >
                          <svg
                            className="w-4 h-4"
                        ...
                        <button
                          ...
                          className="text-foreground/50 hover:text-holo-gold p-2 transition-colors focus:outline-none"
                          title="Düzenle"
                        >
                          <svg
                            className="w-4 h-4"
                        ...
                        <button
                          ...
                          className="text-foreground/50 hover:text-red-500 p-2 transition-colors focus:outline-none"
                          title="Sil"
                        >
                          <svg
                            className="w-4 h-4"
