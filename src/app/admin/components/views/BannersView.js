...
export default function BannersView() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        Swal.fire("Başarılı", `Banner başarıyla ${editingId ? "güncellendi" : "eklendi"}.`, "success");
      ...
      <div className="glass-panel p-8 clip-angled border border-white/5 relative overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            <div>
              <label className="block text-gray-400 text-xs font-bold mb-1 uppercase tracking-wider">Başlık *</label>
              <input type="text" name="title" required value={formData.title} onChange={handleInputChange} className="w-full bg-black/50 border border-white/10 text-white px-4 py-3 text-sm focus:border-neon-pink outline-none transition-colors" placeholder="Banner Başlığı" />
            </div>
            <div>
              <label className="block text-gray-400 text-xs font-bold mb-1 uppercase tracking-wider">Görsel URL veya Yükle *</label>
              <div className="flex gap-2">
                <input type="text" name="imageUrl" required value={formData.imageUrl} onChange={handleInputChange} className="flex-1 bg-black/50 border border-white/10 text-white px-4 py-3 text-sm focus:border-neon-pink outline-none transition-colors" placeholder="/uploads/..." />
                <button type="button" onClick={() => fileInputRef.current?.click()} className="bg-white/10 hover:bg-white/20 text-white px-4 py-3 text-xs uppercase font-bold transition-colors whitespace-nowrap clip-angled">
            ...
            <div>
              <label className="block text-gray-400 text-xs font-bold mb-1 uppercase tracking-wider">Hedef Bağlantı (İsteğe Bağlı)</label>
              <input type="text" name="linkUrl" value={formData.linkUrl} onChange={handleInputChange} className="w-full bg-black/50 border border-white/10 text-white px-4 py-3 text-sm focus:border-neon-pink outline-none transition-colors" placeholder="https://..." />
            </div>
          </div>
          
          <div className="space-y-4">
            <div>
              <label className="block text-gray-400 text-xs font-bold mb-1 uppercase tracking-wider">Sıra No</label>
              <input type="number" name="order" value={formData.order} onChange={handleInputChange} className="w-full bg-black/50 border border-white/10 text-white px-4 py-3 text-sm focus:border-neon-pink outline-none transition-colors" placeholder="0" />
            </div>
            <div className="flex items-center gap-3 pt-4 border-t border-white/5 mt-4">
              <input type="checkbox" name="isActive" checked={formData.isActive} onChange={handleInputChange} id="isActiveCheckbox" className="w-5 h-5 accent-neon-pink" />
              <label htmlFor="isActiveCheckbox" className="text-gray-300 text-sm font-bold cursor-pointer">Aktif Mi?</label>
            </div>
          ...
          <div className="md:col-span-2 pt-4 border-t border-white/10 flex gap-4">
            ...
            {editingId && (
              <button type="button" onClick={cancelEdit} className="border border-white/20 text-white px-8 py-3 uppercase tracking-widest text-xs font-bold hover:bg-white/10 transition-colors clip-angled">
                İptal
              </button>
            )}
          </div>
            ...
            {banners.map((banner) => (
              <div key={banner.id} className={`glass-panel p-4 clip-angled border ${banner.isActive ? 'border-white/10' : 'border-red-500/30 opacity-60'} group hover:border-white/30 transition-all`}>
                <div className="h-40 bg-black/50 mb-4 rounded overflow-hidden flex items-center justify-center relative">
                  ...
                  {!banner.isActive && (
                    <div className="absolute top-2 right-2 bg-red-500 text-white text-[10px] px-2 py-1 font-bold uppercase rounded">Pasif</div>
                  )}
                </div>
                ...
                <div className="flex justify-between items-center text-xs text-gray-500 mb-4">
                  ...
                  {banner.linkUrl && <span className="truncate max-w-[100px]" title={banner.linkUrl}>🔗 Link Var</span>}
                </div>
                
                <div className="flex justify-between border-t border-white/10 pt-3">
                  <button onClick={() => handleEdit(banner)} className="text-gray-400 hover:text-holo-gold text-xs uppercase font-bold tracking-wider transition-colors">
                    Düzenle
                  </button>
                  <button onClick={() => handleDelete(banner.id)} className="text-gray-400 hover:text-red-500 text-xs uppercase font-bold tracking-wider transition-colors">
                    Sil
                  </button>
                </div>
              </div>
