...
export default function OrdersView() {
  ...
  const getStatusStyles = (status) => {
    switch (status) {
      case "Beklemede":
        return { renk: "text-orange-400", border: "border-orange-400/30", bg: "bg-orange-400/10" };
      case "Onaylandı":
        return { renk: "text-blue-400", border: "border-blue-400/30", bg: "bg-blue-400/10" };
      case "Hazırlanıyor":
        return { renk: "text-holo-gold", border: "border-holo-gold/30", bg: "bg-holo-gold/10" };
      case "Kargolandı":
        return { renk: "text-purple-400", border: "border-purple-400/30", bg: "bg-purple-400/10" };
      case "Teslim Edildi":
        return { renk: "text-green-500", border: "border-green-500/30", bg: "bg-green-500/10" };
      case "İptal":
        return { renk: "text-red-500", border: "border-red-500/30", bg: "bg-red-500/10" };
      default:
        return { renk: "text-foreground/50", border: "border-gray-400/30", bg: "bg-gray-400/10" };
    }
  };
  ...
  const handleExport = () => {
    const dataToExport = paginatedOrders.map(
      (o) => `"${o.id}","${o.musteri}","${o.tarih}","${o.tutar}","${o.durum}","${o.odeme}"`
    );
      ...
      <div className="glass-panel p-6 clip-angled border border-glass-border">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 border-b border-glass-border pb-4">
          ...
          <button
            ...
            className="bg-primary hover:bg-secondary text-foreground font-bold py-2 px-6 uppercase tracking-widest text-sm transition-colors clip-angled shadow-[0_0_15px_hsla(var(--primary),0.3)] flex items-center gap-2"
          >
            <svg
              width={20}
              height={20}
      ...
      <div className="glass-panel p-4 clip-angled border border-glass-border flex flex-col lg:flex-row justify-between gap-4 items-center">
        ...
        <div className="flex items-center gap-3 w-full lg:w-auto p-2 bg-foreground/5 border border-glass-border rounded">
          <input
            ...
            checked={
              selectedOrdersIds.length === paginatedOrders.length && paginatedOrders.length > 0
            }
          ...
          <select
            ...
            className="bg-background border border-glass-border text-foreground text-xs px-2 py-1 outline-none"
          >
          ...
          <button
            ...
            disabled={!bulkStatus || selectedOrdersIds.length === 0}
            className="bg-primary/20 text-primary hover:bg-primary hover:text-foreground px-3 py-1 text-xs font-bold uppercase transition-colors disabled:opacity-50"
          >
            Uygula
          </button>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
          <div className="relative">
            ...
            <input
              ...
              className="bg-background border border-glass-border text-foreground pl-9 pr-4 py-1.5 focus:outline-none focus:border-primary text-sm w-48 transition-colors"
            />
          </div>
          ...
          <select
            ...
            className="bg-background border border-glass-border text-foreground px-3 py-1.5 text-sm outline-none"
          >
          ...
          <div className="flex items-center gap-1">
            <input
              ...
              className="bg-background border border-glass-border text-foreground px-2 py-1.5 text-xs outline-none uppercase"
            />
        ...
        <div className="overflow-x-auto w-full">
          <table className="w-full text-left border-collapse min-w-[800px]">
            ...
            <tbody className="divide-y divide-glass-border">
              {loading ? (
                <tr>
                  <td colSpan={8} className="p-8 text-center text-primary animate-pulse">
                    Yükleniyor...
                  </td>
                </tr>
                ...
                paginatedOrders.map((order, i) => (
                  <tr key={i} className="hover:bg-foreground/5 transition-colors group">
                    ...
                    <td
                      className="p-4 font-bold text-foreground text-sm cursor-pointer"
                    ...
                    <td className="p-4">
                      <span
                        className={`text-[10px] px-2 py-1 rounded border uppercase font-bold tracking-widest ${order.odeme === "Ödendi" ? "bg-green-500/10 text-green-500 border-green-500/30" : "bg-orange-500/10 text-orange-500 border-orange-500/30"}`}
                      >
                        {order.odeme}
                      </span>
                    </td>
                    ...
                    <td className="p-4 text-right">
                      <button
                        onClick={() => setSelectedOrder(order)}
                        className="bg-foreground/5 hover:bg-primary hover:text-foreground px-3 py-1.5 text-xs font-bold uppercase tracking-widest transition-colors border border-glass-border"
                      >
                        Detay
                      </button>
                    </td>
          ...
          <div className="border-t border-glass-border p-4 flex justify-between items-center text-foreground/50 text-xs uppercase tracking-wider bg-foreground/5 mt-auto">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="px-3 py-1.5 border border-glass-border hover:border-primary hover:text-primary transition-colors disabled:opacity-30"
              >
                &lt; Önceki
              </button>
              <span className="px-4 py-1.5 bg-background text-foreground font-bold border border-glass-border">
          ...
          <div className="glass-panel w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col relative z-10 animate-fade-in border border-glass-border shadow-2xl">
            ...
            <div className="p-6 border-b border-glass-border flex justify-between items-start bg-foreground/5">
              <div>
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <span
                    className={`px-3 py-1 text-xs border rounded-full font-bold uppercase ${selectedOrder.renk} ${selectedOrder.bg} ${selectedOrder.border}`}
                  >
                    {selectedOrder.durum}
                  </span>
                </div>
                <p className="text-foreground/50 text-sm">
                  {selectedOrder.id} • {selectedOrder.tarih}
                </p>
              </div>
              <button
                onClick={() => setSelectedOrder(null)}
                className="w-8 h-8 flex items-center justify-center bg-foreground/10 hover:bg-danger hover:text-foreground transition-colors rounded"
              >
                <svg
                  width={20}
                  height={20}
            ...
            <div className="flex-1 overflow-y-auto p-6 flex flex-col lg:flex-row gap-8 custom-scrollbar">
              ...
              <div className="flex-1 space-y-6">
                ...
                <div className="bg-foreground/5 border border-glass-border p-4 rounded">
                  <h3 className="text-xs font-bold uppercase text-foreground/50 mb-4 tracking-widest">
                    Sipariş Aşaması
                  </h3>
                  <div className="flex justify-between items-center relative">
                    <div className="absolute left-0 top-1/2 -translate-y-1/2 w-full h-1 bg-glass-border -z-10"></div>
                    {["Beklemede", "Onaylandı", "Hazırlanıyor", "Kargolandı", "Teslim Edildi"].map(
                      (st, idx) => {
                        ...
                        return (
                          <div
                            key={st}
                            className="flex flex-col items-center gap-2 bg-background p-1"
                          >
                            <div
                              className={`w-6 h-6 rounded-full flex items-center justify-center border-2 ${isActive ? "bg-primary border-primary text-foreground shadow-[0_0_10px_hsla(var(--primary),0.5)]" : "bg-background border-glass-border text-foreground/30"} ${isCurrent ? "ring-4 ring-primary/20" : ""}`}
                            >
                              {isActive ? (
                                <svg
                                  width={20}
                            ...
                            <span
                              className={`text-[10px] uppercase font-bold tracking-widest ${isActive ? "text-primary" : "text-foreground/50"}`}
                            >
                              {st}
                            </span>
                          </div>
                ...
                {/* Items */}
                <div>
                  <h3 className="text-xs font-bold uppercase text-foreground/50 mb-3 tracking-widest">
                      ...
                      >
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex gap-2 mt-1">
                            <span className="text-[10px] bg-background border border-glass-border px-1.5 py-0.5 rounded text-foreground/70">
                              Renk: Siyah
                            </span>
                            <span className="text-[10px] bg-background border border-glass-border px-1.5 py-0.5 rounded text-foreground/70">
                              Beden: M
                ...
                <div>
                  ...
                  <textarea
                    placeholder="Bu sipariş ile ilgili sadece yöneticilerin görebileceği notlar ekleyin..."
                    className="w-full bg-foreground/5 border border-glass-border p-3 text-sm focus:border-primary outline-none transition-colors h-24 resize-none"
                  ></textarea>
                  <div className="flex justify-end mt-2">
                    <button className="bg-foreground/10 hover:bg-primary px-4 py-2 text-xs font-bold uppercase tracking-widest transition-colors">
                      Notu Kaydet
                    </button>
                  </div>
                </div>
              ...
              <div className="w-full lg:w-80 space-y-6">
                ...
                <div className="border border-glass-border p-4 bg-foreground/5 rounded">
                  <h3 className="text-xs font-bold uppercase text-foreground/50 mb-3 tracking-widest border-b border-glass-border pb-2">
                    Müşteri Bilgileri
                  </h3>
                ...
                <div className="border border-glass-border p-4 bg-foreground/5 rounded">
                  ...
                  <div className="space-y-3">
                    <div>
                      <div className="text-[10px] text-foreground/50 uppercase mb-1">
                        Kargo Firması
                      </div>
                      <input
                        type="text"
                        ...
                        onChange={(e) =>
                          setSelectedOrder({ ...selectedOrder, carrier: e.target.value })
                        }
                    ...
                    <button
                      ...
                      className="w-full bg-primary/20 text-primary hover:bg-primary hover:text-foreground py-2 text-xs font-bold uppercase tracking-widest transition-colors mt-2"
                    >
            ...
            <div className="p-4 border-t border-glass-border bg-background flex justify-between items-center">
              <button
                ...
                className="bg-foreground/5 hover:bg-foreground/10 text-foreground px-6 py-2 uppercase tracking-widest text-xs font-bold transition-colors border border-glass-border flex items-center gap-2"
              >
                <svg
                  width={20}
                  height={20}
              ...
              <button
                onClick={() => setSelectedOrder(null)}
                className="bg-primary hover:bg-secondary text-foreground px-8 py-2 uppercase tracking-widest text-xs font-bold transition-colors clip-angled"
              >
                Kapat
              </button>
            </div>
